<?php
/**
 * Tasks list page: search, filter, sort, paginate, quick actions.
 *
...
$ptp_result = PTP_Tasks_Repository::get_list(
	array(
		'search'     => $ptp_search,
		'status'     => $ptp_status,
...
$ptp_stats      = PTP_Tasks_Repository::get_stats();
...
if ( ! function_exists( 'ptp_tasks_sort_link' ) ) {
	/**
	 ...
	 */
	function ptp_tasks_sort_link( $column, $label, $current_orderby, $current_order ) {
...
<div class="ptp-page-header">
	<h1><?php esc_html_e( 'Tasks', 'personal-project-tracker' ); ?></h1>
	<a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'page' => 'ptp-tasks', 'action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>">
		<?php esc_html_e( '+ Add New Task', 'personal-project-tracker' ); ?>
	</a>
</div>

<div class="ptp-stats-grid">
	<div class="ptp-stat-tile">
		<span class="ptp-stat-value"><?php echo esc_html( number_format_i18n( $ptp_stats['today'] ) ); ?></span>
	...
	<div class="ptp-stat-tile">
		<span class="ptp-stat-value"><?php echo esc_html( number_format_i18n( $ptp_stats['in_progress'] ) ); ?></span>
...
<div class="ptp-card">
	<form method="get" class="ptp-filter-bar">
		<input type="hidden" name="page" value="ptp-tasks" />

		<input
			type="search"
			name="s"
			value="<?php echo esc_attr( $ptp_search ); ?>"
			placeholder="<?php esc_attr_e( 'Search tasks…', 'personal-project-tracker' ); ?>"
		/>
	...
	<?php if ( empty( $ptp_result['items'] ) ) : ?>
		...
		<div class="ptp-empty-state">
			...
			<?php if ( $ptp_has_filters ) : ?>
				<p><?php esc_html_e( 'No tasks match your search or filters.', 'personal-project-tracker' ); ?></p>
			<?php else : ?>
				<p><?php esc_html_e( 'No tasks yet. Create your first task to get started.', 'personal-project-tracker' ); ?></p>
				<p>
					<a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'page' => 'ptp-tasks', 'action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>">
						<?php esc_html_e( '+ Add New Task', 'personal-project-tracker' ); ?>
					</a>
	...
	<?php else : ?>
		...
		<div class="ptp-table-responsive">
			<table class="widefat striped ptp-tasks-table ptp-responsive-table">
				<thead>
					<tr>
						<th><?php ptp_tasks_sort_link( 'title', __( 'Title', 'personal-project-tracker' ), $ptp_orderby, $ptp_order ); ?></th>
						<th class="ptp-col-optional"><?php esc_html_e( 'Project', 'personal-project-tracker' ); ?></th>
						<th><?php ptp_tasks_sort_link( 'status', __( 'Status', 'personal-project-tracker' ), $ptp_orderby, $ptp_order ); ?></th>
						<th><?php ptp_tasks_sort_link( 'priority', __( 'Priority', 'personal-project-tracker' ), $ptp_orderby, $ptp_order ); ?></th>
						<th class="ptp-col-optional"><?php ptp_tasks_sort_link( 'due_date', __( 'Due Date', 'personal-project-tracker' ), $ptp_orderby, $ptp_order ); ?></th>
						<th><?php esc_html_e( 'Actions', 'personal-project-tracker' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php foreach ( $ptp_result['items'] as $ptp_task ) : ?>
						<?php
						$ptp_view_url  = add_query_arg( array( 'page' => 'ptp-tasks', 'action' => 'view', 'id' => $ptp_task->id ), admin_url( 'admin.php' ) );
						$ptp_edit_url  = add_query_arg( array( 'page' => 'ptp-tasks', 'action' => 'edit', 'id' => $ptp_task->id ), admin_url( 'admin.php' ) );
						$ptp_is_overdue = $ptp_task->due_date && $ptp_task->due_date < current_time( 'Y-m-d' ) && ! in_array( $ptp_task->status, array( 'completed', 'cancelled' ), true );
						$ptp_is_done   = 'completed' === $ptp_task->status;
						?>
						<tr>
							<td data-label="<?php esc_attr_e( 'Title', 'personal-project-tracker' ); ?>">
								<a href="<?php echo esc_url( $ptp_view_url ); ?>"><strong><?php echo esc_html( $ptp_task->title ); ?></strong></a>
							</td>
							<td class="ptp-col-optional" data-label="<?php esc_attr_e( 'Project', 'personal-project-tracker' ); ?>"><?php echo esc_html( $ptp_projects[ (int) $ptp_task->project_id ] ?? '—' ); ?></td>
							<td data-label="<?php esc_attr_e( 'Status', 'personal-project-tracker' ); ?>"><span class="ptp-badge ptp-badge-status-<?php echo esc_attr( $ptp_task->status ); ?>"><?php echo esc_html( $ptp_statuses[ $ptp_task->status ] ?? $ptp_task->status ); ?></span></td>
							<td data-label="<?php esc_attr_e( 'Priority', 'personal-project-tracker' ); ?>"><span class="ptp-badge ptp-badge-priority-<?php echo esc_attr( $ptp_task->priority ); ?>"><?php echo esc_html( $ptp_priorities[ $ptp_task->priority ] ?? $ptp_task->priority ); ?></span></td>
							<td class="ptp-col-optional" data-label="<?php esc_attr_e( 'Due Date', 'personal-project-tracker' ); ?>">
								<?php if ( $ptp_task->due_date ) : ?>
									<span class="<?php echo esc_attr( $ptp_is_overdue ? 'ptp-text-danger' : '' ); ?>">
										<?php echo esc_html( mysql2date( get_option( 'date_format' ), $ptp_task->due_date ) ); ?>
							...
							<td class="ptp-td-actions">
								<div class="ptp-quick-actions">
									...
									<?php if ( 'archived' === $ptp_view ) : ?>
										<button type="button" class="button button-small ptp-js-task-restore" data-id="<?php echo esc_attr( $ptp_task->id ); ?>">
											<?php esc_html_e( 'Restore', 'personal-project-tracker' ); ?>
										</button>
									<?php else : ?>
										<button type="button" class="button button-small ptp-js-task-archive" data-id="<?php echo esc_attr( $ptp_task->id ); ?>">
											<?php esc_html_e( 'Archive', 'personal-project-tracker' ); ?>
										</button>
									<?php endif; ?>
									<button type="button" class="button button-small button-link-delete ptp-js-task-delete" data-id="<?php echo esc_attr( $ptp_task->id ); ?>">
