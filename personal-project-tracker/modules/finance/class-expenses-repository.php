...
/**
 * Class PTP_Expenses_Repository
 *
 * Owns all reads/writes to the expenses table. CRUD only — every
 * calculation that uses expense totals (profit, budget usage, reports)
 * lives in PTP_Finance_Service, which calls get_totals_by_currency() here
 * rather than re-summing rows itself, so the arithmetic exists in exactly
 * one place.
 */
class PTP_Expenses_Repository {

	/**
	 * Columns that may be used to sort the expense list.
	 *
	 * @var string[]
	 */
	...
	/**
	 * Get the fully-prefixed expenses table name.
	 *
	 * @return string
	 */
	public static function get_table() {
		return ptp_table( 'expenses' );
	}
	 ...
	 * only place expense input is validated — both the admin form
	...
	public static function prepare_fields( array $raw ) {
		...
		$expense_date = self::sanitize_date( $raw['expense_date'] ?? '' );

		if ( ! $expense_date ) {
			$errors->add( 'date_required', __( 'A valid expense date is required.', 'personal-project-tracker' ) );
		}
	...
	public static function create( array $raw ) {
		...
		if ( false === $inserted ) {
			ptp_log_error( 'Failed to insert expense: ' . $wpdb->last_error );

			return new WP_Error( 'ptp_db_error', __( 'Could not save the expense. Please try again.', 'personal-project-tracker' ) );
		}
		...
		PTP_Activity_Log::log(
			...
			sprintf( __( 'Logged expense of %s', 'personal-project-tracker' ), ptp_format_currency( $data['amount'], $data['currency'] ) )
		);
	...
	public static function update( $id, array $raw ) {
		...
		if ( ! $existing ) {
			return new WP_Error( 'ptp_not_found', __( 'Expense not found.', 'personal-project-tracker' ) );
		}
		...
		if ( false === $updated ) {
			ptp_log_error( 'Failed to update expense ' . $id . ': ' . $wpdb->last_error );

			return new WP_Error( 'ptp_db_error', __( 'Could not update the expense. Please try again.', 'personal-project-tracker' ) );
		}
		...
		PTP_Activity_Log::log(
			...
			$id,
			/* translators: %s: formatted amount. */
			sprintf( __( 'Updated expense (%s)', 'personal-project-tracker' ), ptp_format_currency( $data['amount'], $data['currency'] ) )
		);
	...
	public static function delete( $id ) {
		...
		if ( ! $deleted ) {
			ptp_log_error( 'Failed to delete expense ' . $id . ': ' . $wpdb->last_error );

			return new WP_Error( 'ptp_db_error', __( 'Could not delete the expense. Please try again.', 'personal-project-tracker' ) );
		}
		...
		PTP_Activity_Log::log(
			...
			$id,
			/* translators: %s: formatted amount. */
			sprintf( __( 'Deleted expense (%s)', 'personal-project-tracker' ), ptp_format_currency( $existing->amount, $existing->currency ) )
		);
	...
	/**
	 * Get a filtered, sorted, paginated list of expenses.
	 *
	 * @param array $args {
	 ...
	 *     @type string $date_from  'Y-m-d' expense_date range start (inclusive).
	 *     @type string $date_to    'Y-m-d' expense_date range end (inclusive).
	...
	public static function get_list( array $args = array() ) {
		...
		$orderby = in_array( $args['orderby'], self::$sortable_columns, true ) ? $args['orderby'] : 'expense_date';
	...
	/**
	 * Sum expense amounts grouped by currency — never combined across
	 * currencies, so a caller can never accidentally treat mismatched
	 * currencies as equal. The one place expense totals are aggregated;
