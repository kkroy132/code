...
define("QLQR_QR_PAD0", 0xEC);
define("QLQR_QR_PAD1", 0x11);
...
class QLQR_QRCode {
    ...
    function __construct() {
        $this->typeNumber = 1;
        $this->errorCorrectLevel = QLQR_QR_ERROR_CORRECT_LEVEL_H;
    ...
    function addData($data, $mode = 0) {
        ...
        if ($mode == 0) {
            $mode = QLQR_QRUtil::getMode($data);
        }

        switch($mode) {

        case QLQR_QR_MODE_NUMBER :
            $this->addDataImpl(new QLQR_QRNumber($data) );
            break;

        case QLQR_QR_MODE_ALPHA_NUM :
            $this->addDataImpl(new QLQR_QRAlphaNum($data) );
            break;

        case QLQR_QR_MODE_8BIT_BYTE :
            $this->addDataImpl(new QLQR_QR8BitByte($data) );
            break;

        case QLQR_QR_MODE_KANJI :
            $this->addDataImpl(new QLQR_QRKanji($data) );
            break;
    ...
    function getBestMaskPattern() {
        ...
        for ($i = 0; $i < 8; $i++) {
            ...
            $lostPoint = QLQR_QRUtil::getLostPoint($this);
    ...
    function makeImpl($test, $maskPattern) {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = QLQR_QRCode::createNullArray($this->moduleCount);
        }
        ...
        $data = QLQR_QRCode::createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
            ...
            while (true) {
                ...
                for ($c = 0; $c < 2; $c++) {
                    ...
                    if ($this->modules[$row][$col - $c] === null) {
                        ...
                        if (QLQR_QRUtil::getMask($maskPattern, $row, $col - $c)) {
                            $dark = !$dark;
                        }
    ...
    function setupPositionAdjustPattern() {
        ...
        $pos = QLQR_QRUtil::getPatternPosition($this->typeNumber);
    ...
    function setupTypeNumber($test) {
        ...
        $bits = QLQR_QRUtil::getBCHTypeNumber($this->typeNumber);
    ...
    function setupTypeInfo($test, $maskPattern) {
        ...
        $bits = QLQR_QRUtil::getBCHTypeInfo($data);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        $rsBlocks = QLQR_QRRSBlock::getRSBlocks($typeNumber, $errorCorrectLevel);

        $buffer = new QLQR_QRBitBuffer();

        for ($i = 0; $i < count($dataArray); $i++) {
            /** @var \QLQR_QRData $data */
            $data = $dataArray[$i];
        ...
        while (true) {
            ...
            $buffer->put(QLQR_QR_PAD0, 8);

            if ($buffer->getLengthInBits() >= $totalDataCount * 8) {
                break;
            }
            $buffer->put(QLQR_QR_PAD1, 8);
        }

        return QLQR_QRCode::createBytes($buffer, $rsBlocks);
    }

    /**
     * @param \QLQR_QRBitBuffer $buffer
     * @param \QLQR_QRRSBlock[] $rsBlocks
     *
     * @return array
     */
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = QLQR_QRCode::createNullArray(count($rsBlocks) );
        $ecdata = QLQR_QRCode::createNullArray(count($rsBlocks) );
        ...
        for ($r = 0; $r < $rsBlockCount; $r++) {
            ...
            $dcdata[$r] = QLQR_QRCode::createNullArray($dcCount);
            ...
            $rsPoly = QLQR_QRUtil::getErrorCorrectPolynomial($ecCount);
            $rawPoly = new QLQR_QRPolynomial($dcdata[$r], $rsPoly->getLength() - 1);

            $modPoly = $rawPoly->mod($rsPoly);
            $ecdata[$r] = QLQR_QRCode::createNullArray($rsPoly->getLength() - 1);
        ...
        $data = QLQR_QRCode::createNullArray($totalCodeCount);
    ...
    static function getMinimumQRCode($data, $errorCorrectLevel) {
        ...
        for ($typeNumber = 1; $typeNumber <= 40; $typeNumber++) {
            if ($length <= QLQR_QRUtil::getMaxLength($typeNumber, $mode, $errorCorrectLevel) ) {
    ...
    // esc_attr() below: not reachable from any caller in this plugin (QrCodeGenerator renders QR
    // codes through GD/SVG paths of its own, never through printHTML()/printSVG()), but escaped
    // anyway since $size is a caller-supplied parameter and this is bundled, reviewable code.
    function printHTML($size = "2px") {

        $style = "border-style:none;border-collapse:collapse;margin:0px;padding:0px;";

        print("<table style='" . esc_attr($style) . "'>");

        for ($r = 0; $r < $this->getModuleCount(); $r++) {

            print("<tr style='" . esc_attr($style) . "'>");

            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c)? "#000000" : "#ffffff";
                print("<td style='" . esc_attr("$style;width:$size;height:$size;background-color:$color") . "'></td>");
            }
    ...
    public function printSVG($size = 2)
    {
        ...
        $height = $width;
        print('<svg width="' . esc_attr($width) . '" height="' . esc_attr($height) . '" viewBox="0 0 ' . esc_attr($width) . ' ' . esc_attr($height) . '" xmlns="http://www.w3.org/2000/svg">');

        for ($r = 0; $r < $this->getModuleCount(); $r++) {
            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c) ? "#000000" : "#ffffff";
                print('<rect x="' . esc_attr($c * $size) . '" y="' . esc_attr($r * $size) . '" width="' . esc_attr($size) . '" height="' . esc_attr($size) . '" fill="' . esc_attr($color) . '" shape-rendering="crispEdges"/>');
            }
        }
...
define("QLQR_QR_G15", (1 << 10) | (1 << 8) | (1 << 5)
    | (1 << 4) | (1 << 2) | (1 << 1) | (1 << 0) );

define("QLQR_QR_G18", (1 << 12) | (1 << 11) | (1 << 10)
    | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2) | (1 << 0) );

define("QLQR_QR_G15_MASK", (1 << 14) | (1 << 12) | (1 << 10)
    | (1 << 4) | (1 << 1) );
...
class QLQR_QRUtil {
    ...
    static function getMaxLength($typeNumber, $mode, $errorCorrectLevel) {
        ...
        case QLQR_QR_ERROR_CORRECT_LEVEL_L : $e = 0; break;
        case QLQR_QR_ERROR_CORRECT_LEVEL_M : $e = 1; break;
        case QLQR_QR_ERROR_CORRECT_LEVEL_Q : $e = 2; break;
        case QLQR_QR_ERROR_CORRECT_LEVEL_H : $e = 3; break;
        default :
            trigger_error("e:$errorCorrectLevel", E_USER_ERROR);
        }

        switch($mode) {
        case QLQR_QR_MODE_NUMBER    : $m = 0; break;
        case QLQR_QR_MODE_ALPHA_NUM : $m = 1; break;
        case QLQR_QR_MODE_8BIT_BYTE : $m = 2; break;
        case QLQR_QR_MODE_KANJI     : $m = 3; break;
        default :
    ...
    static function getErrorCorrectPolynomial($errorCorrectLength) {
        ...
        $a = new QLQR_QRPolynomial(array(1) );

        for ($i = 0; $i < $errorCorrectLength; $i++) {
            $a = $a->multiply(new QLQR_QRPolynomial(array(1, QLQR_QRMath::gexp($i) ) ) );
        }
    ...
    static function getMask($maskPattern, $i, $j) {
        ...
        case QLQR_QR_MASK_PATTERN000 : return ($i + $j) % 2 == 0;
        case QLQR_QR_MASK_PATTERN001 : return $i % 2 == 0;
        case QLQR_QR_MASK_PATTERN010 : return $j % 3 == 0;
        case QLQR_QR_MASK_PATTERN011 : return ($i + $j) % 3 == 0;
        case QLQR_QR_MASK_PATTERN100 : return (floor($i / 2) + floor($j / 3) ) % 2 == 0;
        case QLQR_QR_MASK_PATTERN101 : return ($i * $j) % 2 + ($i * $j) % 3 == 0;
        case QLQR_QR_MASK_PATTERN110 : return ( ($i * $j) % 2 + ($i * $j) % 3) % 2 == 0;
        case QLQR_QR_MASK_PATTERN111 : return ( ($i * $j) % 3 + ($i + $j) % 2) % 2 == 0;
    ...
    static function getMode($s) {
        if (QLQR_QRUtil::isAlphaNum($s) ) {
            if (QLQR_QRUtil::isNumber($s) ) {
                return QLQR_QR_MODE_NUMBER;
            }
            return QLQR_QR_MODE_ALPHA_NUM;
        } else if (QLQR_QRUtil::isKanji($s) ) {
            return QLQR_QR_MODE_KANJI;
        } else {
            return QLQR_QR_MODE_8BIT_BYTE;
        }
    }
    ...
    static function isNumber($s) {
        for ($i = 0; $i < strlen($s); $i++) {
            $c = ord($s[$i]);
            if (!(QLQR_QRUtil::toCharCode('0') <= $c && $c <= QLQR_QRUtil::toCharCode('9') ) ) {
                return false;
            }
        }
        return true;
    ...
    static function isAlphaNum($s) {
        for ($i = 0; $i < strlen($s); $i++) {
            $c = ord($s[$i]);
            if (!(QLQR_QRUtil::toCharCode('0') <= $c && $c <= QLQR_QRUtil::toCharCode('9') )
                && !(QLQR_QRUtil::toCharCode('A') <= $c && $c <= QLQR_QRUtil::toCharCode('Z') )
                    && strpos(" $%*+-./:", $s[$i]) === false) {
                return false;
            }
        }
    ...
    static function getBCHTypeInfo($data) {
        $d = $data << 10;
        while (QLQR_QRUtil::getBCHDigit($d) - QLQR_QRUtil::getBCHDigit(QLQR_QR_G15) >= 0) {
            $d ^= (QLQR_QR_G15 << (QLQR_QRUtil::getBCHDigit($d) - QLQR_QRUtil::getBCHDigit(QLQR_QR_G15) ) );
        }
        return ( ($data << 10) | $d) ^ QLQR_QR_G15_MASK;
    }

    static function getBCHTypeNumber($data) {
        $d = $data << 12;
        while (QLQR_QRUtil::getBCHDigit($d) - QLQR_QRUtil::getBCHDigit(QLQR_QR_G18) >= 0) {
            $d ^= (QLQR_QR_G18 << (QLQR_QRUtil::getBCHDigit($d) - QLQR_QRUtil::getBCHDigit(QLQR_QR_G18) ) );
        }
        return ($data << 12) | $d;
    }
...
class QLQR_QRRSBlock {
    ...
    static function getRSBlocks($typeNumber, $errorCorrectLevel) {
        ...
        $rsBlock = QLQR_QRRSBlock::getRsBlockTable($typeNumber, $errorCorrectLevel);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            for ($j = 0; $j < $count; $j++) {
                $list[] = new QLQR_QRRSBlock($totalCount, $dataCount);
            }
        }
...
class QLQR_QRNumber extends QLQR_QRData {

    function __construct($data) {
        parent::__construct(QLQR_QR_MODE_NUMBER, $data);
    }
    ...
    function write(&$buffer) {
        ...
        while ($i + 2 < strlen($data) ) {
            $num = QLQR_QRNumber::parseInt(substr($data, $i, 3) );
            $buffer->put($num, 10);
            $i += 3;
        }
        ...
        if ($i < strlen($data) ) {
            ...
            if (strlen($data) - $i == 1) {
                $num = QLQR_QRNumber::parseInt(substr($data, $i, $i + 1) );
                $buffer->put($num, 4);
            } else if (strlen($data) - $i == 2) {
                $num = QLQR_QRNumber::parseInt(substr($data, $i, $i + 2) );
                $buffer->put($num, 7);
            }
        }
    }
    ...
    static function parseInt($s) {
        ...
        $num = 0;
        for ($i = 0; $i < strlen($s); $i++) {
            $num = $num * 10 + QLQR_QRNumber::parseIntAt(ord($s[$i]) );
        }
        return $num;
    }
    ...
    static function parseIntAt($c) {
        ...
        if (QLQR_QRUtil::toCharCode('0') <= $c && $c <= QLQR_QRUtil::toCharCode('9') ) {
            return $c - QLQR_QRUtil::toCharCode('0');
        }
...
class QLQR_QRKanji extends QLQR_QRData {

    function __construct($data) {
        parent::__construct(QLQR_QR_MODE_KANJI, $data);
    }
...
class QLQR_QRAlphaNum extends QLQR_QRData {

    function __construct($data) {
        parent::__construct(QLQR_QR_MODE_ALPHA_NUM, $data);
    }
    ...
    function write(&$buffer) {
        ...
        while ($i + 1 < strlen($c) ) {
            $buffer->put(QLQR_QRAlphaNum::getCode(ord($c[$i]) ) * 45
                + QLQR_QRAlphaNum::getCode(ord($c[$i + 1]) ), 11);
            $i += 2;
        }

        if ($i < strlen($c) ) {
            $buffer->put(QLQR_QRAlphaNum::getCode(ord($c[$i])), 6);
        }
    }

    static function getCode($c) {

        if (QLQR_QRUtil::toCharCode('0') <= $c
                && $c <= QLQR_QRUtil::toCharCode('9') ) {
            return $c - QLQR_QRUtil::toCharCode('0');
        } else if (QLQR_QRUtil::toCharCode('A') <= $c
                && $c <= QLQR_QRUtil::toCharCode('Z') ) {
            return $c - QLQR_QRUtil::toCharCode('A') + 10;
        } else {
            switch ($c) {
            case QLQR_QRUtil::toCharCode(' ') : return 36;
            case QLQR_QRUtil::toCharCode('$') : return 37;
            case QLQR_QRUtil::toCharCode('%') : return 38;
            case QLQR_QRUtil::toCharCode('*') : return 39;
            case QLQR_QRUtil::toCharCode('+') : return 40;
            case QLQR_QRUtil::toCharCode('-') : return 41;
            case QLQR_QRUtil::toCharCode('.') : return 42;
            case QLQR_QRUtil::toCharCode('/') : return 43;
            case QLQR_QRUtil::toCharCode(':') : return 44;
            default :
                trigger_error("illegal char : $c", E_USER_ERROR);
            }
        }
...
class QLQR_QR8BitByte extends QLQR_QRData {

    function __construct($data) {
        parent::__construct(QLQR_QR_MODE_8BIT_BYTE, $data);
    }
...
abstract class QLQR_QRData {
    ...
    function getLengthInBits($type) {
        ...
        if (1 <= $type && $type < 10) {
            ...
            switch($this->mode) {
            case QLQR_QR_MODE_NUMBER     : return 10;
            case QLQR_QR_MODE_ALPHA_NUM     : return 9;
            case QLQR_QR_MODE_8BIT_BYTE    : return 8;
            case QLQR_QR_MODE_KANJI      : return 8;
            default :
            ...
            switch($this->mode) {
            case QLQR_QR_MODE_NUMBER     : return 12;
            case QLQR_QR_MODE_ALPHA_NUM     : return 11;
            case QLQR_QR_MODE_8BIT_BYTE    : return 16;
            case QLQR_QR_MODE_KANJI      : return 10;
            default :
                trigger_error("mode:$this->mode", E_USER_ERROR);
            }
            ...
            switch($this->mode) {
            case QLQR_QR_MODE_NUMBER     : return 14;
            case QLQR_QR_MODE_ALPHA_NUM    : return 13;
            case QLQR_QR_MODE_8BIT_BYTE    : return 16;
            case QLQR_QR_MODE_KANJI      : return 12;
            default :
                trigger_error("mode:$this->mode", E_USER_ERROR);
            }
...
class QLQR_QRMath {
    ...
    static function init() {
        ...
        self::$QR_MATH_EXP_TABLE = QLQR_QRMath::createNumArray(256);
        ...
        self::$QR_MATH_LOG_TABLE = QLQR_QRMath::createNumArray(256);
...
class QLQR_QRPolynomial {
    ...
    function __construct($num, $shift = 0) {
        ...
        $this->num = QLQR_QRMath::createNumArray(count($num) - $offset + $shift);
    ...
    function toLogString() {
        ...
        for ($i = 0; $i < $this->getLength(); $i++) {
            ...
            $buffer .= QLQR_QRMath::glog($this->get($i) );
        }
    ...
    /**
     * @param \QLQR_QRPolynomial $e
     *
     * @return \QLQR_QRPolynomial
     */
    function multiply($e) {

        $num = QLQR_QRMath::createNumArray($this->getLength() + $e->getLength() - 1);

        for ($i = 0; $i < $this->getLength(); $i++) {
            $vi = QLQR_QRMath::glog($this->get($i) );

            for ($j = 0; $j < $e->getLength(); $j++) {
                $num[$i + $j] ^= QLQR_QRMath::gexp($vi + QLQR_QRMath::glog($e->get($j) ) );
            }
        }

        return new QLQR_QRPolynomial($num);
    }

    /**
     * @param \QLQR_QRPolynomial $e
     *
     * @return $this|\QLQR_QRPolynomial
     */
    function mod($e) {
        ...
        $ratio = QLQR_QRMath::glog($this->get(0) ) - QLQR_QRMath::glog($e->get(0) );

        $num = QLQR_QRMath::createNumArray($this->getLength() );
        for ($i = 0; $i < $this->getLength(); $i++) {
            $num[$i] = $this->get($i);
        }

        for ($i = 0; $i < $e->getLength(); $i++) {
            $num[$i] ^= QLQR_QRMath::gexp(QLQR_QRMath::glog($e->get($i) ) + $ratio);
        }

        $newPolynomial = new QLQR_QRPolynomial($num);
...
define("QLQR_QR_MODE_NUMBER", 1 << 0);
define("QLQR_QR_MODE_ALPHA_NUM", 1 << 1);
define("QLQR_QR_MODE_8BIT_BYTE", 1 << 2);
define("QLQR_QR_MODE_KANJI", 1 << 3);

//---------------------------------------------------------------
// MaskPattern
//---------------------------------------------------------------

define("QLQR_QR_MASK_PATTERN000", 0);
define("QLQR_QR_MASK_PATTERN001", 1);
define("QLQR_QR_MASK_PATTERN010", 2);
define("QLQR_QR_MASK_PATTERN011", 3);
define("QLQR_QR_MASK_PATTERN100", 4);
define("QLQR_QR_MASK_PATTERN101", 5);
define("QLQR_QR_MASK_PATTERN110", 6);
define("QLQR_QR_MASK_PATTERN111", 7);

//---------------------------------------------------------------
// ErrorCorrectLevel

// 7%.
define("QLQR_QR_ERROR_CORRECT_LEVEL_L", 1);
// 15%.
define("QLQR_QR_ERROR_CORRECT_LEVEL_M", 0);
// 25%.
define("QLQR_QR_ERROR_CORRECT_LEVEL_Q", 3);
// 30%.
define("QLQR_QR_ERROR_CORRECT_LEVEL_H", 2);
