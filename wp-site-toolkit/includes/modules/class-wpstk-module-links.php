...
class WPSTK_Module_Links extends WPSTK_Module {
	...
	public function get_label() {
		return __( 'Links', 'wp-site-toolkit' );
	}
	...
	public function get_description() {
		return __( 'Broken links, redirects, internal linking, orphaned content and recorded 404 requests.', 'wp-site-toolkit' );
	}
	...
	public function get_tasks( $settings ) {
		$tasks = array(
			array(
				'id'    => 'collect',
				'label' => __( 'Collecting links from your content', 'wp-site-toolkit' ),
			),
			array(
				'id'    => 'internal',
				'label' => __( 'Checking internal links', 'wp-site-toolkit' ),
			),
		);
		...
		if ( ! empty( $settings['scan_external'] ) ) {
			$tasks[] = array(
				'id'    => 'external',
				'label' => __( 'Checking external links', 'wp-site-toolkit' ),
			);
		}
	...
	private function broken_check( $kind, $results, $collect ) {
		$label = __( 'Broken internal links', 'wp-site-toolkit' );

		if ( empty( $results[ $kind ]['checked'] ) ) {
			return $this->skipped( 'broken_internal', $label, __( 'No internal links were found in the content that was scanned.', 'wp-site-toolkit' ) );
		}
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail'     => sprintf(
						...
						__( '%1$s — linked from "%2$s"', 'wp-site-toolkit' ),
						$row['message'],
						'' !== $row['title'] ? $row['title'] : __( '(no title)', 'wp-site-toolkit' )
					),
					'url'        => $row['url'],
					'link'       => $row['post'] > 0 ? (string) get_edit_post_link( $row['post'], 'raw' ) : '',
					'link_label' => $row['post'] > 0 ? __( 'Edit source page', 'wp-site-toolkit' ) : '',
				)
			);
		}
			...
			return $this->check(
				array(
					...
					'summary' => sprintf(
						...
						_n( '%d internal link responded normally.', 'All %d internal links responded normally.', $checked, 'wp-site-toolkit' ),
						$checked
					),
					'why'     => __( 'Internal links that lead nowhere waste visitors\' time and stop search engines from reaching your pages.', 'wp-site-toolkit' ),
				)
			);
		}
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					__( '%1$d of %2$d internal links did not respond normally (%3$s).', 'wp-site-toolkit' ),
					$count,
					$checked,
					$this->describe_buckets( $buckets )
				),
				'why'         => __( 'Internal links that lead nowhere waste visitors\' time and stop search engines from reaching your pages.', 'wp-site-toolkit' ),
				'action'      => __( 'Open each source page and either fix the address or remove the link. Timeouts are sometimes temporary — re-run the audit to confirm.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $count,
			)
		);
	...
	private function external_check( $results, $collect, $settings ) {
		$label = __( 'External links', 'wp-site-toolkit' );
		...
		if ( empty( $results['external']['checked'] ) ) {
			return $this->skipped( 'broken_external', $label, __( 'No external links were found in the content that was scanned.', 'wp-site-toolkit' ) );
		}
		...
		if ( 0 === $count ) {
			return $this->check(
				array(
					...
					'summary' => sprintf(
						/* translators: %d: number of links checked. */
						_n( '%d external link responded normally.', 'All %d external links responded normally.', $checked, 'wp-site-toolkit' ),
						$checked
					),
					'why'     => __( 'Links to sites that have disappeared make your content look neglected.', 'wp-site-toolkit' ),
				)
			);
		}
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					__( '%1$d of %2$d external links did not respond normally (%3$s).', 'wp-site-toolkit' ),
					$count,
					$checked,
					$this->describe_buckets( $buckets )
				),
				'why'         => __( 'Links to sites that have disappeared make your content look neglected.', 'wp-site-toolkit' ),
				'action'      => __( 'Check the listed addresses in a browser. Some sites block automated requests, so treat "access denied" results as a hint rather than proof.', 'wp-site-toolkit' ),
				'note'        => __( 'External sites sometimes rate limit or block automated requests. Verify before removing a link.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $count,
			)
		);
	...
	private function redirect_check( $results ) {
		$label     = __( 'Redirects and redirect chains', 'wp-site-toolkit' );
		$redirects = array();
		...
		if ( 0 === $checked ) {
			return $this->skipped( 'redirects', $label, __( 'No links were available to check.', 'wp-site-toolkit' ) );
		}
		...
		if ( empty( $redirects ) ) {
			return $this->passed(
				'redirects',
				$label,
				__( 'No redirected links were found in your content.', 'wp-site-toolkit' ),
				__( 'Linking straight to the final address saves a round trip for every visitor.', 'wp-site-toolkit' )
			);
		}
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail'     => sprintf(
						...
						_n( '%1$d redirect to %2$s', '%1$d redirects to %2$s', (int) $row['hops'], 'wp-site-toolkit' ),
						(int) $row['hops'],
			...
			array(
				...
				'summary'     => $chains > 0
					? sprintf(
						...
						__( '%1$d links redirect before reaching their destination, and %2$d of those go through more than one hop.', 'wp-site-toolkit' ),
						count( $redirects ),
						$chains
					)
					: sprintf(
						/* translators: %d: number of redirected links. */
						_n( '%d link redirects once before reaching its destination.', '%d links redirect once before reaching their destination.', count( $redirects ), 'wp-site-toolkit' ),
						count( $redirects )
					),
				'why'         => __( 'Each redirect adds a round trip. Chains of two or more are slower still and can lose referral information.', 'wp-site-toolkit' ),
				'action'      => __( 'Update the links to point straight at the final address.', 'wp-site-toolkit' ),
				'items'       => $items,
	...
	private function internal_linking_checks( $collect ) {
		$label_density = __( 'Internal linking', 'wp-site-toolkit' );
		$label_orphan  = __( 'Orphaned content', 'wp-site-toolkit' );

		if ( empty( $collect['posts_scanned'] ) ) {
			return array(
				$this->skipped( 'internal_linking', $label_density, __( 'No published content was available to analyse.', 'wp-site-toolkit' ) ),
				$this->skipped( 'orphaned_content', $label_orphan, __( 'No published content was available to analyse.', 'wp-site-toolkit' ) ),
			);
		}
		...
		if ( empty( $collect['covered_all'] ) ) {
			$note = sprintf(
				...
				__( 'Based on the first %1$d of %2$d published entries, so a page linked from content outside that range may appear here incorrectly.', 'wp-site-toolkit' ),
				$scanned,
		...
		foreach ( (array) $collect['post_links_out'] as $post_id => $count ) {
			if ( $count < 1 ) {
				$few_links[] = WPSTK_Check::post_item(
					...
					__( 'No links to other pages on this site', 'wp-site-toolkit' )
				);
			}
		...
		foreach ( (array) $collect['inbound'] as $post_id => $count ) {
			...
			if ( count( $orphans ) < WPSTK_Check::MAX_ITEMS ) {
				$orphans[] = WPSTK_Check::post_item(
					$post_id,
					isset( $collect['post_titles'][ $post_id ] ) ? $collect['post_titles'][ $post_id ] : '',
					__( 'No links found from other scanned pages', 'wp-site-toolkit' )
				);
			}
		}
			...
			array(
				...
				'summary'     => $few_total > 0
					? sprintf(
						...
						__( '%1$d of %2$d entries link to nothing else on this site. %3$d internal links were found in total.', 'wp-site-toolkit' ),
						$few_total,
						$scanned,
					...
					: sprintf(
						...
						__( 'All %1$d entries link to at least one other page. %2$d internal links were found in total.', 'wp-site-toolkit' ),
						$scanned,
						(int) $collect['internal_total']
					),
				'why'         => __( 'Internal links help readers find related content and help search engines understand how your pages relate to each other.', 'wp-site-toolkit' ),
				'action'      => __( 'Add a few relevant links from these entries to other pages on your site.', 'wp-site-toolkit' ),
				'note'        => $note,
				'items'       => $few_links,
			...
			array(
				...
				'summary'     => $orphan_total > 0
					? sprintf(
						...
						__( '%1$d of %2$d entries are not linked from any other scanned entry.', 'wp-site-toolkit' ),
						$orphan_total,
						$scanned
					)
					: __( 'Every analysed entry is linked from at least one other entry.', 'wp-site-toolkit' ),
				'why'         => __( 'Content that nothing links to is harder for both visitors and search engines to discover.', 'wp-site-toolkit' ),
				'action'      => __( 'Link to these entries from related content, a menu or a listing page.', 'wp-site-toolkit' ),
				'note'        => trim( $note . ' ' . __( 'Links from menus, widgets and archive pages are not counted, so some entries listed here may still be reachable.', 'wp-site-toolkit' ) ),
				'items'       => $orphans,
	...
	private function not_found_check( $settings ) {
		$label = __( '404 requests', 'wp-site-toolkit' );

		if ( empty( $settings['monitor_404'] ) ) {
			return $this->skipped( 'not_found_log', $label, __( '404 monitoring is turned off in the settings.', 'wp-site-toolkit' ) );
		}
		...
		if ( 0 === $total ) {
			return $this->passed(
				'not_found_log',
				$label,
				__( 'No 404 requests have been recorded in the last 30 days.', 'wp-site-toolkit' ),
				__( 'Repeated 404s usually mean a page moved without a redirect, or something links to an address that never existed.', 'wp-site-toolkit' )
			);
		}
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail' => sprintf(
						...
						_n( '%1$d request, last seen %2$s', '%1$d requests, last seen %2$s', (int) $row['hits'], 'wp-site-toolkit' ),
						(int) $row['hits'],
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					_n( '%d distinct address returned a 404 in the last 30 days.', '%d distinct addresses returned a 404 in the last 30 days.', $total, 'wp-site-toolkit' ),
					$total
				),
				'why'         => __( 'Repeated 404s usually mean a page moved without a redirect, or something links to an address that never existed.', 'wp-site-toolkit' ),
				'action'      => __( 'Review the list under Links → 404 monitor. Redirect the addresses that used to work and ignore the obvious bot noise.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $total,
			)
		);
		...
		foreach ( $buckets as $type => $count ) {
			...
			switch ( $type ) {
				case 'not_found':
					...
					$parts[] = sprintf( _n( '%d not found', '%d not found', $count, 'wp-site-toolkit' ), $count );
					break;
				case 'forbidden':
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d access denied', '%d access denied', $count, 'wp-site-toolkit' ), $count );
					break;
				case 'client':
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d client error', '%d client errors', $count, 'wp-site-toolkit' ), $count );
					break;
				case 'server':
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d server error', '%d server errors', $count, 'wp-site-toolkit' ), $count );
					break;
				case 'timeout':
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d timed out', '%d timed out', $count, 'wp-site-toolkit' ), $count );
					break;
				case 'redirect':
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d redirect problem', '%d redirect problems', $count, 'wp-site-toolkit' ), $count );
					break;
				default:
					/* translators: %d: number of links. */
					$parts[] = sprintf( _n( '%d unreachable', '%d unreachable', $count, 'wp-site-toolkit' ), $count );
					break;
			}
		}
