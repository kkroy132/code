...
class WPSTK_Module_Performance extends WPSTK_Module {
	...
	public function get_label() {
		return __( 'Performance', 'wp-site-toolkit' );
	}
	...
	public function get_description() {
		return __( 'Database size, revisions, autoloaded options, media weight and a basic response time reading. Basic local check only.', 'wp-site-toolkit' );
	}
	...
	public function get_tasks( $settings ) {
		return array(
			array(
				'id'    => 'database',
				'label' => __( 'Measuring the database', 'wp-site-toolkit' ),
			),
			array(
				'id'    => 'response',
				'label' => __( 'Measuring server response time', 'wp-site-toolkit' ),
			),
		);
	}
	...
	private function content_check( $database ) {
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail'     => sprintf(
						...
						_n( '%d published entry', '%d published entries', (int) $count, 'wp-site-toolkit' ),
						(int) $count
					),
					'link'       => admin_url( 'edit.php?post_type=' . rawurlencode( $type ) ),
					'link_label' => __( 'View', 'wp-site-toolkit' ),
				)
			);
		}
		...
		return $this->check(
			array(
				'id'      => 'content_volume',
				'status'  => 'passed',
				'label'   => __( 'Published content', 'wp-site-toolkit' ),
				'weight'  => 0.5,
				'summary' => sprintf(
					/* translators: %d: number of published entries. */
					_n( '%d published entry across all public post types.', '%d published entries across all public post types.', $total, 'wp-site-toolkit' ),
					$total
				),
				'why'     => __( 'A reference figure that helps put the other numbers on this page in context.', 'wp-site-toolkit' ),
				'items'   => $items,
			)
		);
	}
	...
	private function database_check( $database ) {
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail' => sprintf(
						...
						__( '%1$s, about %2$s rows', 'wp-site-toolkit' ),
		...
		return $this->check(
			array(
				'id'      => 'database_size',
				'status'  => $status,
				'label'   => __( 'Database size', 'wp-site-toolkit' ),
				'weight'  => 0.8,
				'summary' => sprintf(
					/* translators: 1: total database size, 2: reclaimable size. */
					__( 'The WordPress tables use %1$s, of which about %2$s is reclaimable space.', 'wp-site-toolkit' ),
					size_format( (int) $database['total_bytes'] ),
					size_format( $overhead )
				),
				'why'     => __( 'A large database is not a problem on its own, but reclaimable space suggests tables that would benefit from being optimised.', 'wp-site-toolkit' ),
				'action'  => 'passed' === $status ? '' : __( 'Ask your host to optimise the tables, or use your hosting control panel\'s database tools. WP Site Toolkit does not modify your tables.', 'wp-site-toolkit' ),
				'note'    => __( 'The largest tables are listed below.', 'wp-site-toolkit' ),
				'items'   => $items,
			)
		);
	}
	...
	private function revisions_check( $database ) {
		...
		if ( $revisions < 500 ) {
			return $this->passed(
				'revisions',
				__( 'Post revisions', 'wp-site-toolkit' ),
				sprintf(
					/* translators: %s: number of revisions. */
					__( '%s revisions are stored, which is a normal amount.', 'wp-site-toolkit' ),
					number_format_i18n( $revisions )
				),
				__( 'Revisions let you undo edits. They only become a concern when there are tens of thousands of them.', 'wp-site-toolkit' )
			);
		}
		...
		if ( null === $limit ) {
			$note = __( 'WP_POST_REVISIONS is not set, so WordPress keeps every revision. Setting it in wp-config.php limits how many are kept from now on.', 'wp-site-toolkit' );
		} elseif ( false === $limit ) {
			$note = __( 'Revisions are disabled by WP_POST_REVISIONS, so this total will not grow.', 'wp-site-toolkit' );
		}
		...
		return $this->check(
			array(
				...
				'label'   => __( 'Post revisions', 'wp-site-toolkit' ),
				'summary' => sprintf(
					/* translators: %s: number of revisions. */
					__( '%s post revisions are stored.', 'wp-site-toolkit' ),
					number_format_i18n( $revisions )
				),
				'why'     => __( 'Every revision is a row in the posts table. Large numbers make backups bigger and some queries slower.', 'wp-site-toolkit' ),
				'action'  => __( 'Limit future revisions with define( \'WP_POST_REVISIONS\', 5 ); in wp-config.php. Removing existing revisions is a job for a dedicated cleanup tool — WP Site Toolkit does not delete content.', 'wp-site-toolkit' ),
				'note'    => $note,
			)
		);
	}
	...
	private function autoload_check( $database ) {
		...
		if ( $bytes < self::AUTOLOAD_WARN ) {
			return $this->check(
				array(
					...
					'status'  => 'passed',
					'label'   => __( 'Autoloaded options', 'wp-site-toolkit' ),
					'weight'  => 1.5,
					'summary' => sprintf(
						/* translators: 1: total size, 2: number of options. */
						__( '%1$s of options load on every request, across %2$s entries.', 'wp-site-toolkit' ),
						size_format( $bytes ),
						number_format_i18n( $count )
					),
					'why'     => __( 'Autoloaded options are read from the database on every single page load, so keeping them small helps every request.', 'wp-site-toolkit' ),
					'items'   => $items,
				)
			);
		}
		...
		return $this->check(
			array(
				...
				'why'     => __( 'Autoloaded options are read from the database on every single page load. Once this grows past roughly 800 KB it starts adding measurable time to every request.', 'wp-site-toolkit' ),
				'action'  => __( 'Look at the largest entries below. They usually belong to a plugin — often one that was removed without cleaning up. Check with the plugin author before deleting anything.', 'wp-site-toolkit' ),
				'note'    => __( 'The largest autoloaded options are listed below.', 'wp-site-toolkit' ),
				'items'   => $items,
			)
		);
	}
	...
	private function cleanup_check( $database ) {
		...
		$rows = array(
			array( __( 'Trashed posts and pages', 'wp-site-toolkit' ), (int) $database['trashed'], admin_url( 'edit.php?post_status=trash&post_type=post' ) ),
			array( __( 'Auto-draft entries', 'wp-site-toolkit' ), (int) $database['autodrafts'], '' ),
			array( __( 'Spam comments', 'wp-site-toolkit' ), (int) $database['spam_comments'], admin_url( 'edit-comments.php?comment_status=spam' ) ),
			array( __( 'Stored transients', 'wp-site-toolkit' ), (int) $database['transients'], '' ),
		);
		...
		foreach ( $rows as $row ) {
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'link'       => $row[2],
					'link_label' => '' !== $row[2] ? __( 'View', 'wp-site-toolkit' ) : '',
				)
			);
		}
		...
		if ( $total < 200 ) {
			return $this->passed(
				'database_clutter',
				__( 'Database clutter', 'wp-site-toolkit' ),
				__( 'Trash, auto-drafts, spam and transients are all at reasonable levels.', 'wp-site-toolkit' ),
				__( 'These rows accumulate quietly and are safe to clear once you no longer need them.', 'wp-site-toolkit' )
			);
		}
		...
		return $this->check(
			array(
				...
				'label'   => __( 'Database clutter', 'wp-site-toolkit' ),
				'weight'  => 0.8,
				'summary' => sprintf(
					/* translators: %s: number of rows. */
					__( '%s rows of trash, auto-drafts, spam comments and transients are stored.', 'wp-site-toolkit' ),
					number_format_i18n( $total )
				),
				'why'     => __( 'None of this is dangerous, but it makes the database bigger than it needs to be and slows down backups.', 'wp-site-toolkit' ),
				'action'  => __( 'Empty the trash and the spam queue when you are sure you no longer need them. Expired transients are cleared by WordPress automatically.', 'wp-site-toolkit' ),
				'items'   => $items,
			)
		);
	}
	...
	private function media_weight_check( $all_state ) {
		...
		if ( empty( $media['count'] ) ) {
			return $this->skipped(
				'media_weight',
				__( 'Media weight', 'wp-site-toolkit' ),
				__( 'No images were measured in this scan.', 'wp-site-toolkit' )
			);
		}
		...
		return $this->check(
			array(
				'id'      => 'media_weight',
				'status'  => $status,
				'label'   => __( 'Media weight', 'wp-site-toolkit' ),
				'summary' => sprintf(
					/* translators: 1: number of images, 2: total size, 3: average size. */
					__( '%1$s original image files use %2$s, an average of %3$s each.', 'wp-site-toolkit' ),
					number_format_i18n( $count ),
					size_format( $bytes ),
					size_format( $average )
				),
				'why'     => __( 'The average size of your originals is a good indicator of how heavy your pages will feel to visitors.', 'wp-site-toolkit' ),
				'action'  => 'passed' === $status ? '' : __( 'Compress images before uploading them. See the Images section for the specific files involved.', 'wp-site-toolkit' ),
				'note'    => __( 'Original files only. The smaller sizes WordPress generates are not counted.', 'wp-site-toolkit' ),
			)
		);
	}
	...
	private function response_check( $response ) {
		$label = __( 'Server response time', 'wp-site-toolkit' );
		$note  = __( 'Basic local check only. This is a request from your server to itself, so it excludes network latency, DNS and browser rendering. It is not a substitute for a page speed or Core Web Vitals test.', 'wp-site-toolkit' );

		if ( empty( $response ) || null === $response['average'] ) {
			return $this->skipped( 'response_time', $label, __( 'The front page could not be loaded from the server, so no timing was recorded.', 'wp-site-toolkit' ) );
		}
		...
		return $this->check(
			array(
				...
				'summary' => sprintf(
					...
					__( 'The front page took %s seconds to respond on average.', 'wp-site-toolkit' ),
					number_format_i18n( $average, 2 )
				),
				'why'     => __( 'A slow first byte delays everything that follows, however well optimised the rest of the page is.', 'wp-site-toolkit' ),
				'action'  => 'passed' === $status ? '' : __( 'Consider a caching plugin, a persistent object cache, or a faster hosting plan. Deactivating plugins one at a time is the usual way to find a slow one.', 'wp-site-toolkit' ),
				'note'    => $note,
			)
		);
	}
	...
	private function object_cache_check( $database ) {
		...
		if ( ! empty( $database['object_cache'] ) ) {
			return $this->passed(
				'object_cache',
				__( 'Persistent object cache', 'wp-site-toolkit' ),
				__( 'A persistent object cache is active.', 'wp-site-toolkit' ),
				__( 'A persistent object cache saves repeating the same database queries on every request.', 'wp-site-toolkit' )
			);
		}
		...
		return $this->check(
			array(
				'id'      => 'object_cache',
				'status'  => 'recommendation',
				'label'   => __( 'Persistent object cache', 'wp-site-toolkit' ),
				'weight'  => 0.5,
				'summary' => __( 'No persistent object cache was detected.', 'wp-site-toolkit' ),
				'why'     => __( 'Without one, WordPress repeats the same database queries on every request. Most sites work fine without it; busy sites benefit noticeably.', 'wp-site-toolkit' ),
				'action'  => __( 'Ask your host whether Redis or Memcached is available. This is optional, not a fault.', 'wp-site-toolkit' ),
			)
		);
	}
}
