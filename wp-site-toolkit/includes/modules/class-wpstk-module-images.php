...
class WPSTK_Module_Images extends WPSTK_Module {
	...
	public function get_label() {
		return __( 'Images', 'wp-site-toolkit' );
	}
	...
	public function get_description() {
		return __( 'Alt text, oversized files, unusual dimensions, total media usage and potentially unused attachments.', 'wp-site-toolkit' );
	}
	...
	public function get_tasks( $settings ) {
		return array(
			array(
				'id'    => 'content',
				'label' => __( 'Checking images used in your content', 'wp-site-toolkit' ),
			),
			array(
				'id'    => 'media',
				'label' => __( 'Measuring the media library', 'wp-site-toolkit' ),
			),
		);
	}
	...
	private function task_media( $offset, $state, $settings ) {
		...
		foreach ( $images as $image ) {
			...
			$title  = '' !== trim( (string) $image['post_title'] ) ? $image['post_title'] : __( '(untitled)', 'wp-site-toolkit' );
	...
	private function alt_text_check( $content, $media ) {
		$label = __( 'Image alt text', 'wp-site-toolkit' );

		if ( empty( $content['images_found'] ) && empty( $media['count'] ) ) {
			return $this->skipped( 'alt_text', $label, __( 'No images were found in your content or media library.', 'wp-site-toolkit' ) );
		}
		...
		foreach ( (array) $this->state_get( $content, 'samples', array() ) as $row ) {
			$items[] = WPSTK_Check::post_item(
				...
				sprintf(
					...
					_n( '%d image without an alt attribute', '%d images without an alt attribute', (int) $row[2], 'wp-site-toolkit' ),
					(int) $row[2]
				)
			);
		}
				...
				array(
					...
					'summary' => $library_missing > 0
						? sprintf(
							...
							__( 'All %1$d images in your content have an alt attribute, but %2$d media library items have no alt text saved.', 'wp-site-toolkit' ),
							$found,
							$library_missing
						)
						: sprintf(
							/* translators: %d: number of images. */
							_n( 'The %d image in your content has an alt attribute.', 'All %d images in your content have an alt attribute.', $found, 'wp-site-toolkit' ),
							$found
						),
					'why'     => __( 'Alt text describes an image to people using a screen reader and to search engines. Purely decorative images should keep an empty alt attribute.', 'wp-site-toolkit' ),
					'action'  => $library_missing > 0 ? __( 'Add alt text in the media library so it is filled in automatically the next time you insert those images.', 'wp-site-toolkit' ) : '',
					'items'   => $library_missing > 0 ? array(
						WPSTK_Check::item(
							array(
								'label'      => __( 'Media library', 'wp-site-toolkit' ),
								'detail'     => sprintf(
									/* translators: %d: number of images. */
									_n( '%d image without alt text', '%d images without alt text', $library_missing, 'wp-site-toolkit' ),
									$library_missing
								),
								'link'       => admin_url( 'upload.php' ),
								'link_label' => __( 'Open media library', 'wp-site-toolkit' ),
							)
						),
					) : array(),
				)
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					__( '%1$d of %2$d images in your content have no alt attribute at all. A further %3$d use an empty alt, which is correct for decorative images.', 'wp-site-toolkit' ),
					$missing,
					$found,
					(int) $this->state_get( $content, 'decorative', 0 )
				),
				'why'         => __( 'Alt text describes an image to people using a screen reader and to search engines. Without it, the image is simply skipped.', 'wp-site-toolkit' ),
				'action'      => __( 'Open the listed entries and describe each image in a short sentence. Leave the alt empty only when the image is decorative.', 'wp-site-toolkit' ),
				'items'       => $items,
	...
	private function large_files_check( $media, $settings ) {
		$label = __( 'Large image files', 'wp-site-toolkit' );

		if ( empty( $media['count'] ) ) {
			return $this->skipped( 'large_images', $label, __( 'The media library contains no images.', 'wp-site-toolkit' ) );
		}
		...
		if ( 0 === $count ) {
			return $this->passed(
				...
				sprintf(
					...
					__( 'No image is larger than %s.', 'wp-site-toolkit' ),
					size_format( $threshold )
				),
				__( 'Large images are the most common cause of slow pages on WordPress sites.', 'wp-site-toolkit' )
			);
		}
				...
				array(
					...
					'detail'     => $row[3] > 0
						? sprintf(
							...
							__( '%1$s — %2$d × %3$d pixels', 'wp-site-toolkit' ),
					...
					'link_label' => __( 'View image', 'wp-site-toolkit' ),
				)
			);
		}
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					__( '%1$d of %2$d images are larger than %3$s.', 'wp-site-toolkit' ),
					$count,
					(int) $media['count'],
					size_format( $threshold )
				),
				'why'         => __( 'Large images are the most common cause of slow pages. They cost your visitors time and data, especially on mobile connections.', 'wp-site-toolkit' ),
				'action'      => __( 'Re-save these images at a smaller size or compress them, then replace them in the media library. WP Site Toolkit never modifies or deletes your files.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $count,
			)
		);
	...
	private function dimensions_check( $media, $settings ) {
		$label = __( 'Image dimensions', 'wp-site-toolkit' );

		if ( empty( $media['count'] ) ) {
			return $this->skipped( 'image_dimensions', $label, __( 'The media library contains no images.', 'wp-site-toolkit' ) );
		}
		...
		if ( 0 === $count ) {
			return $this->passed(
				...
				sprintf(
					...
					__( 'No image is wider or taller than %d pixels.', 'wp-site-toolkit' ),
					$limit
				),
				__( 'Images far larger than they are displayed waste bandwidth on every page view.', 'wp-site-toolkit' )
			);
		}
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'detail'     => sprintf(
						...
						__( '%1$d × %2$d pixels', 'wp-site-toolkit' ),
						(int) $row[2],
						(int) $row[3]
					),
					'url'        => (string) wp_get_attachment_url( (int) $row[0] ),
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					_n( '%1$d image is wider or taller than %2$d pixels.', '%1$d images are wider or taller than %2$d pixels.', $count, 'wp-site-toolkit' ),
					$count,
					$limit
				),
				'why'         => __( 'Very large originals are rarely displayed at full size. WordPress generates smaller versions, but the original is still stored and sometimes still served.', 'wp-site-toolkit' ),
				'action'      => __( 'Resize the originals before uploading them next time. Existing files can stay as they are if the generated sizes are what your theme uses.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $count,
			)
		);
	...
	private function library_check( $media ) {
		$label = __( 'Media library size', 'wp-site-toolkit' );

		if ( empty( $media['count'] ) ) {
			return $this->skipped( 'media_report', $label, __( 'The media library contains no images.', 'wp-site-toolkit' ) );
		}
		...
		return $this->check(
			array(
				...
				'summary'     => sprintf(
					...
					__( '%1$d images use %2$s in total (measured for %3$d files).', 'wp-site-toolkit' ),
					(int) $media['count'],
					size_format( (int) $media['bytes'] ),
					(int) $media['measured']
				),
				'why'         => __( 'This is a reference figure, not a problem. It counts original files only, not the smaller sizes WordPress generates.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => count( $items ),
			)
		);
	...
	private function unused_check( $content, $media ) {
		$label = __( 'Potentially unused media', 'wp-site-toolkit' );

		if ( empty( $media['ids'] ) ) {
			return $this->skipped( 'unused_media', $label, __( 'The media library contains no images.', 'wp-site-toolkit' ) );
		}
		...
		if ( ! empty( $media['ids_truncated'] ) ) {
			return $this->skipped(
				'unused_media',
				$label,
				__( 'Skipped because this media library is larger than the 3,000 files this check inspects in a single scan.', 'wp-site-toolkit' )
			);
		}
		...
		if ( empty( $content['covered_all'] ) ) {
			return $this->skipped(
				...
				sprintf(
					...
					__( 'Skipped because only %1$d of %2$d published entries were analysed. Media usage can only be judged reliably when every entry is scanned — raise the "Maximum posts per scan" setting to enable this check.', 'wp-site-toolkit' ),
		...
		if ( 0 === $count ) {
			return $this->passed(
				'unused_media',
				$label,
				__( 'Every image in the library is referenced by your content, a featured image, the site icon or the site logo.', 'wp-site-toolkit' ),
				__( 'Unused files still take up storage and make the library harder to search.', 'wp-site-toolkit' )
			);
		}
		...
		return $this->check(
			array(
				...
				'why'         => __( 'Unused files take up storage and make the media library harder to search.', 'wp-site-toolkit' ),
				'action'      => __( 'Review each file before doing anything with it. WP Site Toolkit never deletes media.', 'wp-site-toolkit' ),
				'note'        => __( 'Potentially unused only. Images used by page builders, custom fields, theme options, widgets or CSS backgrounds cannot be detected here, so treat this list as a starting point rather than a verdict.', 'wp-site-toolkit' ),
				'items'       => $items,
				'items_total' => $count,
			)
		);
