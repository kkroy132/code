...
class WPSTK_View {
	...
	public static function meter( $score ) {
		...
		if ( null === $score ) {
			...
			$label = sprintf(
				...
				__( 'Score: %d out of 100', 'wp-site-toolkit' ),
				(int) $score
			);
		}
	...
	public static function summary_tiles( $counts ) {
		$tiles = array(
			'critical'       => __( 'Critical issues', 'wp-site-toolkit' ),
			'warning'        => __( 'Warnings', 'wp-site-toolkit' ),
			'recommendation' => __( 'Recommendations', 'wp-site-toolkit' ),
			'passed'         => __( 'Passed checks', 'wp-site-toolkit' ),
			'skipped'        => __( 'Not checked', 'wp-site-toolkit' ),
		);
	...
	public static function check_list( $checks, $show_section = false ) {
		if ( empty( $checks ) ) {
			self::empty_state( __( 'Nothing to show here yet.', 'wp-site-toolkit' ) );
		...
		foreach ( $checks as $check ) {
			...
			if ( (int) $check['items_total'] > 0 ) {
				echo '<span class="wpstk-check__count">' . esc_html(
					sprintf(
						...
						_n( '%s item', '%s items', (int) $check['items_total'], 'wp-site-toolkit' ),
			...
			if ( '' !== $check['summary'] ) {
				echo '<p class="wpstk-check__finding"><strong>' . esc_html__( 'What was found:', 'wp-site-toolkit' ) . '</strong> ' . esc_html( $check['summary'] ) . '</p>';
			}

			if ( '' !== $check['why'] ) {
				echo '<p class="wpstk-check__why"><strong>' . esc_html__( 'Why it matters:', 'wp-site-toolkit' ) . '</strong> ' . esc_html( $check['why'] ) . '</p>';
			}

			if ( '' !== $check['action'] ) {
				echo '<p class="wpstk-check__action"><strong>' . esc_html__( 'What to do:', 'wp-site-toolkit' ) . '</strong> ' . esc_html( $check['action'] ) . '</p>';
			}
	...
	public static function item_table( $check ) {
		...
		echo '<thead><tr>';
		echo '<th scope="col">' . esc_html__( 'Item', 'wp-site-toolkit' ) . '</th>';
		echo '<th scope="col">' . esc_html__( 'Detail', 'wp-site-toolkit' ) . '</th>';
		echo '<th scope="col" class="wpstk-items__actions">' . esc_html__( 'Action', 'wp-site-toolkit' ) . '</th>';
		echo '</tr></thead><tbody>';
		...
		foreach ( $check['items'] as $item ) {
			...
			if ( '' !== $item['link'] ) {
				printf(
					...
					esc_url( $item['link'] ),
					esc_html( '' !== $item['link_label'] ? $item['link_label'] : __( 'View', 'wp-site-toolkit' ) )
				);
			} elseif ( '' !== $item['url'] ) {
				printf(
					'<a class="button button-small" href="%1$s" target="_blank" rel="noopener noreferrer">%2$s</a>',
					esc_url( $item['url'] ),
					esc_html__( 'Open', 'wp-site-toolkit' )
				);
			} else {
				echo '&mdash;';
			}
		...
		if ( $total > $shown ) {
			echo '<p class="wpstk-check__more">' . esc_html(
				sprintf(
					...
					__( 'Showing %1$s of %2$s affected items.', 'wp-site-toolkit' ),
	...
	public static function runner( $running, $compact = false ) {
		...
		if ( $can_run ) {
			printf(
				...
				esc_html__( 'Run Full Site Audit', 'wp-site-toolkit' )
			);

			printf(
				'<button type="button" class="button wpstk-runner__cancel" data-wpstk-cancel%1$s>%2$s</button>',
				$is_running ? '' : ' hidden="hidden"',
				esc_html__( 'Cancel', 'wp-site-toolkit' )
			);
		} else {
			echo '<p class="description">' . esc_html__( 'You can view results, but you do not have permission to start an audit.', 'wp-site-toolkit' ) . '</p>';
		}
