...
class WPSTK_Post_Insights {
	...
	public static function add_column( $columns ) {
		...
		$columns[ self::COLUMN_ID ] = __( 'Site Toolkit', 'wp-site-toolkit' );
	...
	public static function render_meta_box( $post ) {
		...
		if ( WPSTK_Security::can( 'run' ) ) {
			echo '<p class="wpstk-insights__footer"><a href="' . esc_url( WPSTK_Admin::page_url( 'wp-site-toolkit-audit' ) ) . '">' . esc_html__( 'Run a full site audit for complete results', 'wp-site-toolkit' ) . '</a></p>';
		}
	...
	private static function title_signal( $post_row, $post_meta ) {
		...
		if ( '' === $value ) {
			return array(
				'status' => 'critical',
				'text'   => __( 'Missing title', 'wp-site-toolkit' ),
			);
		}
		...
		if ( $length < WPSTK_Module_SEO::TITLE_MIN ) {
			return array(
				...
				'text'   => sprintf( __( 'Title is short (%d characters)', 'wp-site-toolkit' ), $length ),
			);
		}
		...
		if ( $length > WPSTK_Module_SEO::TITLE_MAX ) {
			return array(
				'status' => 'warning',
				/* translators: %d: number of characters. */
				'text'   => sprintf( __( 'Title is long (%d characters)', 'wp-site-toolkit' ), $length ),
			);
		}
		...
		return array(
			'status' => 'passed',
			/* translators: %d: number of characters. */
			'text'   => sprintf( __( 'Title length is good (%d characters)', 'wp-site-toolkit' ), $length ),
		);
	}
	...
	private static function description_signal( $post_row, $post_meta ) {
		...
		if ( 'none' === $description['source'] || '' === $value ) {
			return array(
				'status' => 'warning',
				'text'   => __( 'No meta description', 'wp-site-toolkit' ),
			);
		}
		...
		if ( $length < WPSTK_Module_SEO::DESC_MIN ) {
			return array(
				'status' => 'recommendation',
				/* translators: %d: number of characters. */
				'text'   => sprintf( __( 'Description is short (%d characters)', 'wp-site-toolkit' ), $length ),
			);
		}
		...
		if ( $length > WPSTK_Module_SEO::DESC_MAX ) {
			return array(
				'status' => 'recommendation',
				/* translators: %d: number of characters. */
				'text'   => sprintf( __( 'Description is long (%d characters)', 'wp-site-toolkit' ), $length ),
			);
		}
		...
		return array(
			'status' => 'passed',
			/* translators: %d: number of characters. */
			'text'   => sprintf( __( 'Description length is good (%d characters)', 'wp-site-toolkit' ), $length ),
		);
	}
	...
	private static function image_signal( $post ) {
		...
		if ( 0 === $found_images ) {
			return array(
				'status' => 'passed',
				'text'   => __( 'No images to check', 'wp-site-toolkit' ),
			);
		}
		...
		if ( $missing_alt > 0 ) {
			return array(
				...
				'text'   => sprintf( _n( '%d image missing alt text', '%d images missing alt text', $missing_alt, 'wp-site-toolkit' ), $missing_alt ),
			);
		}
		...
		return array(
			'status' => 'passed',
			/* translators: %d: number of images. */
			'text'   => sprintf( _n( '%d image has alt text', '%d images have alt text', $found_images, 'wp-site-toolkit' ), $found_images ),
		);
	}
	...
	private static function render_badges( $signals ) {
		...
		$labels = array(
			'title'       => __( 'Title', 'wp-site-toolkit' ),
			'description' => __( 'Description', 'wp-site-toolkit' ),
			'images'      => __( 'Images', 'wp-site-toolkit' ),
		);
	...
	private static function render_rows( $signals ) {
		if ( empty( $signals ) ) {
			echo '<p>' . esc_html__( 'Nothing to check yet.', 'wp-site-toolkit' ) . '</p>';
		...
		$labels = array(
			'title'       => __( 'Title', 'wp-site-toolkit' ),
			'description' => __( 'Meta description', 'wp-site-toolkit' ),
			'images'      => __( 'Image alt text', 'wp-site-toolkit' ),
		);
