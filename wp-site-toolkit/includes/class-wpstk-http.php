...
class WPSTK_HTTP {
	...
	public static function check_url( $url, $timeout ) {
		...
		if ( ! self::is_requestable( $url ) ) {
			$result['message'] = __( 'The address could not be parsed as a web URL.', 'wp-site-toolkit' );
		...
		while ( $hops <= self::MAX_REDIRECTS ) {
			...
			if ( $status >= 300 && $status < 400 ) {
				...
				if ( ! $location ) {
					...
					$result['message'] = __( 'Redirect response without a destination.', 'wp-site-toolkit' );
				...
				if ( ! self::is_requestable( $next ) || in_array( $next, $result['chain'], true ) ) {
					$result['type']    = 'redirect';
					$result['message'] = __( 'Redirect loop or unsupported redirect target.', 'wp-site-toolkit' );
		...
		if ( $hops > self::MAX_REDIRECTS ) {
			$result['type']    = 'redirect';
			$result['message'] = __( 'Too many redirects.', 'wp-site-toolkit' );
		}
	...
	public static function describe( $type, $status = 0 ) {
		switch ( $type ) {
			case 'ok':
				...
				return sprintf( __( 'Reachable (HTTP %d)', 'wp-site-toolkit' ), (int) $status );
			case 'redirect':
				return __( 'Redirect could not be resolved', 'wp-site-toolkit' );
			case 'not_found':
				/* translators: %d: HTTP status code. */
				return sprintf( __( 'Not found (HTTP %d)', 'wp-site-toolkit' ), (int) $status );
			case 'forbidden':
				/* translators: %d: HTTP status code. */
				return sprintf( __( 'Access denied (HTTP %d)', 'wp-site-toolkit' ), (int) $status );
			case 'client':
				/* translators: %d: HTTP status code. */
				return sprintf( __( 'Client error (HTTP %d)', 'wp-site-toolkit' ), (int) $status );
			case 'server':
				/* translators: %d: HTTP status code. */
				return sprintf( __( 'Server error (HTTP %d)', 'wp-site-toolkit' ), (int) $status );
			case 'timeout':
				return __( 'The request timed out', 'wp-site-toolkit' );
			default:
				return __( 'The address could not be reached', 'wp-site-toolkit' );
		}
	}
	...
	public static function fetch( $url, $timeout ) {
		...
		if ( ! self::is_requestable( $url ) ) {
			$out['message'] = __( 'The address could not be parsed as a web URL.', 'wp-site-toolkit' );
