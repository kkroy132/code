...
class WPSTK_Audit {
	...
	public function start( $trigger = 'manual' ) {
		...
		if ( $existing && 'running' === $existing['status'] ) {
			if ( ! $this->is_stale( $existing ) ) {
				return new WP_Error(
					'wpstk_scan_running',
					__( 'An audit is already running. Wait for it to finish or cancel it first.', 'wp-site-toolkit' )
				);
			}
		...
		if ( empty( $queue ) ) {
			return new WP_Error( 'wpstk_no_tasks', __( 'No audit tasks are available.', 'wp-site-toolkit' ) );
		}
		...
		if ( $scan_id <= 0 ) {
			return new WP_Error(
				...
				__( 'The audit could not be started because the scan record could not be saved.', 'wp-site-toolkit' )
			);
		}
	...
	public function step() {
		...
		if ( ! $state || 'running' !== $state['status'] ) {
			return new WP_Error( 'wpstk_no_scan', __( 'No audit is currently running.', 'wp-site-toolkit' ) );
		}
	...
	private function finish( $state ) {
		...
		return array(
			...
			'label'       => __( 'Audit complete', 'wp-site-toolkit' ),
			'module'      => '',
		);
	}
	...
	public function progress( $state ) {
		...
		$current = is_array( $current ) ? $current : array(
			'module' => '',
			'label'  => __( 'Finishing up', 'wp-site-toolkit' ),
		);
