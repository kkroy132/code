...
<div class="wpstk-layout">
	<div class="wpstk-layout__main">
		<div class="wpstk-card wpstk-hero wpstk-hero--<?php echo esc_attr( $wpstk_grade['key'] ); ?>">
			<div class="wpstk-hero__score">
				<p class="wpstk-hero__label"><?php echo esc_html__( 'Website Health', 'wp-site-toolkit' ); ?></p>
				...
				<?php if ( $wpstk_latest ) : ?>
					<p class="wpstk-hero__meta">
						...
						printf(
							...
							esc_html__( 'Last audit: %s ago', 'wp-site-toolkit' ),
							esc_html( human_time_diff( (int) strtotime( $wpstk_latest['finished_at'] . ' UTC' ), time() ) )
						);
						?>
						&middot;
						<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'wp-site-toolkit-reports', array( 'scan_id' => (int) $wpstk_latest['id'] ) ) ); ?>">
							<?php echo esc_html__( 'View full report', 'wp-site-toolkit' ); ?>
						</a>
					</p>
				<?php endif; ?>
			</div>
			...
			<?php foreach ( $wpstk_modules as $wpstk_module_id => $wpstk_module ) : ?>
				...
				WPSTK_View::score_card(
					array(
						...
						'url'      => WPSTK_Admin::page_url( 'wp-site-toolkit-' . $wpstk_module_id ),
		...
		<?php if ( $wpstk_latest ) : ?>
			...
			<h2 class="wpstk-section-title"><?php echo esc_html__( 'Needs attention first', 'wp-site-toolkit' ); ?></h2>

			<?php if ( empty( $wpstk_attention ) ) : ?>
				<?php
				WPSTK_View::empty_state(
					__( 'No critical issues or warnings were found in the latest audit. Recommendations are listed in each section.', 'wp-site-toolkit' )
				);
				?>
			<?php else : ?>
				...
				<p>
					<a class="button" href="<?php echo esc_url( WPSTK_Admin::page_url( 'wp-site-toolkit-reports', array( 'scan_id' => (int) $wpstk_latest['id'] ) ) ); ?>">
						<?php echo esc_html__( 'See every finding', 'wp-site-toolkit' ); ?>
					</a>
				</p>
			<?php endif; ?>
		<?php else : ?>
			<h2 class="wpstk-section-title"><?php echo esc_html__( 'Getting started', 'wp-site-toolkit' ); ?></h2>
			<?php
			WPSTK_View::empty_state(
				__( 'No audit has been run yet. Start one to see where your site stands — it runs in small batches so it will not lock up your admin area.', 'wp-site-toolkit' ),
				__( 'Open Full Audit', 'wp-site-toolkit' ),
	...
	<div class="wpstk-layout__side">
		<div class="wpstk-card">
			<h2 class="wpstk-card__title"><?php echo esc_html__( 'How scoring works', 'wp-site-toolkit' ); ?></h2>
			<p><?php echo esc_html__( 'Each section runs a fixed set of checks. A check that passes scores full marks, a recommendation scores most of them, a warning scores half, and a critical finding scores none. Checks that could not run are excluded.', 'wp-site-toolkit' ); ?></p>
			<p><?php echo esc_html__( 'Scores are a guide to where to spend your time. They are not a guarantee of search rankings or of security.', 'wp-site-toolkit' ); ?></p>
		</div>

		<div class="wpstk-card">
			<h2 class="wpstk-card__title"><?php echo esc_html__( 'Privacy', 'wp-site-toolkit' ); ?></h2>
			<p><?php echo esc_html__( 'Everything is processed on this site. No content, statistics or personal data is sent anywhere, and there is no telemetry or advertising.', 'wp-site-toolkit' ); ?></p>
			<p>
				<?php echo esc_html__( 'During an audit the plugin loads pages from your own site to inspect the markup it serves.', 'wp-site-toolkit' ); ?>
				<?php
				if ( empty( $view['settings']['scan_external'] ) ) {
					echo esc_html__( 'External link checking is off, so no third-party site is contacted.', 'wp-site-toolkit' );
				} else {
					echo esc_html__( 'External link checking is on, so sites you link to are requested to see whether they still respond.', 'wp-site-toolkit' );
				}
				?>
			</p>
			<p>
				<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'wp-site-toolkit-settings' ) ); ?>">
					<?php echo esc_html__( 'Review settings', 'wp-site-toolkit' ); ?>
				</a>
			</p>
		</div>
		...
		if ( count( $wpstk_history ) > 1 ) :
			?>
			<div class="wpstk-card">
				<h2 class="wpstk-card__title"><?php echo esc_html__( 'Recent scores', 'wp-site-toolkit' ); ?></h2>
