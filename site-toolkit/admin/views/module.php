...
if ( ! $wpstk_module ) {
	WPSTK_View::empty_state( __( 'That section is not available.', 'site-toolkit' ) );
...
<div class="wpstk-card wpstk-module-head">
	<div class="wpstk-module-head__intro">
		...
		<?php if ( 'security' === $wpstk_module_id ) : ?>
			<p class="wpstk-callout"><?php echo esc_html__( 'This is a Basic Security Health Check. It reads configuration and response headers only. It does not test for vulnerabilities and is not a substitute for a full security review.', 'site-toolkit' ); ?></p>
		<?php endif; ?>

		<?php if ( 'performance' === $wpstk_module_id ) : ?>
			<p class="wpstk-callout"><?php echo esc_html__( 'Basic local check only. These measurements are taken on the server and do not replace a real page speed or Core Web Vitals test taken from a visitor\'s browser.', 'site-toolkit' ); ?></p>
		<?php endif; ?>
	</div>
	...
	<div class="wpstk-module-head__score">
		...
		WPSTK_View::score_card(
			array(
				'title'  => __( 'Section score', 'site-toolkit' ),
				'score'  => $wpstk_score,
				'counts' => $wpstk_counts,
			)
		);
...
<?php if ( 'links' === $wpstk_module_id ) : ?>
	<nav class="nav-tab-wrapper wpstk-tabs">
		<a class="nav-tab <?php echo in_array( $wpstk_tab, array( '404', 'redirects' ), true ) ? '' : 'nav-tab-active'; ?>" href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-links' ) ); ?>">
			<?php echo esc_html__( 'Link results', 'site-toolkit' ); ?>
		</a>
		<a class="nav-tab <?php echo '404' === $wpstk_tab ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-links', array( 'tab' => '404' ) ) ); ?>">
			<?php echo esc_html__( '404 monitor', 'site-toolkit' ); ?>
		</a>
		<a class="nav-tab <?php echo 'redirects' === $wpstk_tab ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-links', array( 'tab' => 'redirects' ) ) ); ?>">
			<?php echo esc_html__( 'Redirects', 'site-toolkit' ); ?>
		</a>
	</nav>
<?php endif; ?>
...
<?php if ( 'links' === $wpstk_module_id && '404' === $wpstk_tab ) : ?>
	...
	<div class="wpstk-card">
		<h2 class="wpstk-card__title"><?php echo esc_html__( '404 monitor', 'site-toolkit' ); ?></h2>

		<?php if ( empty( $view['settings']['monitor_404'] ) ) : ?>
			<p class="wpstk-callout">
				<?php echo esc_html__( '404 monitoring is currently turned off, so nothing new is being recorded.', 'site-toolkit' ); ?>
				<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-settings' ) ); ?>"><?php echo esc_html__( 'Turn it on', 'site-toolkit' ); ?></a>
			</p>
		<?php endif; ?>
		...
		<p class="description">
			...
			printf(
				...
				esc_html__( 'The requested address, the referring URL and a hit counter are stored — nothing else. No IP addresses or user agents are recorded. Up to %s distinct addresses are kept.', 'site-toolkit' ),
		...
		<?php if ( empty( $wpstk_rows ) ) : ?>
			<?php WPSTK_View::empty_state( __( 'No 404 requests have been recorded yet.', 'site-toolkit' ) ); ?>
		<?php else : ?>
			<div class="wpstk-table-wrap">
				<table class="wp-list-table widefat striped">
					<thead>
						<tr>
							<th scope="col"><?php echo esc_html__( 'Requested address', 'site-toolkit' ); ?></th>
							<th scope="col"><?php echo esc_html__( 'Referrer', 'site-toolkit' ); ?></th>
							<th scope="col"><?php echo esc_html__( 'Hits', 'site-toolkit' ); ?></th>
							<th scope="col"><?php echo esc_html__( 'First seen', 'site-toolkit' ); ?></th>
							<th scope="col"><?php echo esc_html__( 'Last seen', 'site-toolkit' ); ?></th>
							<th scope="col"><?php echo esc_html__( 'Action', 'site-toolkit' ); ?></th>
						</tr>
					</thead>
					<tbody>
							...
							<tr>
								...
								<td>
									<?php if ( WPSTK_Security::can( 'manage' ) ) : ?>
										<a class="button button-small" href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-links', array( 'tab' => 'redirects', 'source' => wp_parse_url( $wpstk_row['url'], PHP_URL_PATH ) ) ) ); ?>">
											<?php echo esc_html__( 'Redirect', 'site-toolkit' ); ?>
										</a>
										...
										?>
										"><?php echo esc_html__( 'Remove', 'site-toolkit' ); ?></a>
									<?php else : ?>
										&mdash;
									<?php endif; ?>
								</td>
			...
			<?php if ( $wpstk_pages > 1 ) : ?>
				<div class="tablenav"><div class="tablenav-pages">
					...
					echo wp_kses_post(
						paginate_links(
							array(
								'base'      => WPSTK_Admin::page_url( 'site-toolkit-links', array( 'tab' => '404' ) ) . '%_%',
								'format'    => '&paged=%#%',
								'current'   => $wpstk_paged,
								'total'     => $wpstk_pages,
								'prev_text' => '&laquo;',
			...
			<?php if ( WPSTK_Security::can( 'manage' ) ) : ?>
				<p>
					...
					?>
					"><?php echo esc_html__( 'Clear the 404 log', 'site-toolkit' ); ?></a>
				</p>
			<?php endif; ?>
		<?php endif; ?>
	</div>
...
<?php else : ?>
	<?php if ( ! $wpstk_latest ) : ?>
		...
		WPSTK_View::empty_state(
			__( 'No audit has been completed yet, so there is nothing to show for this section.', 'site-toolkit' ),
			__( 'Run an audit', 'site-toolkit' ),
	...
	<?php else : ?>
		<p class="wpstk-result-meta">
			...
			printf(
				...
				esc_html__( 'From the audit finished on %s.', 'site-toolkit' ),
