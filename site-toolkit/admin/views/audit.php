...
<div class="wpstk-card wpstk-runner-card">
	<h2 class="wpstk-card__title"><?php echo esc_html__( 'Full site audit', 'site-toolkit' ); ?></h2>
	<p><?php echo esc_html__( 'The audit runs every available check and works through your content in small batches, so it will not tie up the admin area or overload your server. You can leave this page open and watch the progress.', 'site-toolkit' ); ?></p>
	...
	<p class="description">
		...
		printf(
			...
			esc_html__( 'Current limits: up to %1$s published entries, %2$s links and %3$s rendered pages per audit.', 'site-toolkit' ),
		...
		?>
		<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-settings' ) ); ?>"><?php echo esc_html__( 'Change limits', 'site-toolkit' ); ?></a>
	</p>
</div>

<h2 class="wpstk-section-title"><?php echo esc_html__( 'What the audit covers', 'site-toolkit' ); ?></h2>
			...
			<p>
				<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-' . $wpstk_module_id ) ); ?>">
					...
					printf(
						...
						esc_html__( 'Open %s results', 'site-toolkit' ),
...
<?php if ( $wpstk_latest ) : ?>
	...
	<h2 class="wpstk-section-title">
		<?php echo esc_html__( 'Latest results', 'site-toolkit' ); ?>
	...
	<p>
		...
		printf(
			...
			esc_html__( 'Total checks: %s', 'site-toolkit' ),
...
<?php else : ?>
	<?php WPSTK_View::empty_state( __( 'No audit has been completed yet. Use the button above to run the first one.', 'site-toolkit' ) ); ?>
<?php endif; 
