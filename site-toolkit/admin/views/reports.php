...
if ( ! $wpstk_scan ) {
	WPSTK_View::empty_state(
		__( 'No completed audit is stored yet. Run one and the report will appear here.', 'site-toolkit' ),
...
<div class="wpstk-report-actions wpstk-no-print">
	...
	?>
	"><?php echo esc_html__( 'Export CSV', 'site-toolkit' ); ?></a>

	<button type="button" class="button" data-wpstk-print><?php echo esc_html__( 'Print report', 'site-toolkit' ); ?></button>

	<?php if ( WPSTK_Security::can( 'manage' ) ) : ?>
		<a class="button wpstk-button-danger" data-wpstk-confirm="<?php echo esc_attr__( 'Delete this scan and its findings?', 'site-toolkit' ); ?>" href="
		<?php
		echo esc_url(
			wp_nonce_url(
				add_query_arg(
		...
		?>
		"><?php echo esc_html__( 'Delete scan', 'site-toolkit' ); ?></a>
	<?php endif; ?>
</div>
...
<div class="wpstk-card wpstk-report-head">
	<div>
		<h2 class="wpstk-card__title">
			...
			printf(
				...
				esc_html__( 'Report from %s', 'site-toolkit' ),
		...
		<p class="description">
			...
			printf(
				...
				esc_html__( 'Started %1$s, completed in %2$s seconds, %3$s checks run.', 'site-toolkit' ),
				esc_html( 'cron' === $wpstk_scan['trigger_type'] ? __( 'automatically', 'site-toolkit' ) : __( 'manually', 'site-toolkit' ) ),
	...
	<div class="wpstk-report-head__score">
		...
		WPSTK_View::score_card(
			array(
				'title'    => __( 'Website Health', 'site-toolkit' ),
				'score'    => $wpstk_overall,
...
<div class="wpstk-table-wrap">
	<table class="wp-list-table widefat striped">
		<thead>
			<tr>
				<th scope="col"><?php echo esc_html__( 'Section', 'site-toolkit' ); ?></th>
				<th scope="col"><?php echo esc_html__( 'Score', 'site-toolkit' ); ?></th>
				<?php if ( $wpstk_previous ) : ?>
					<th scope="col"><?php echo esc_html__( 'Previous', 'site-toolkit' ); ?></th>
					<th scope="col"><?php echo esc_html__( 'Change', 'site-toolkit' ); ?></th>
				<?php endif; ?>
		...
		<tbody>
			<?php foreach ( $wpstk_modules as $wpstk_module_id => $wpstk_module ) : ?>
				...
				<tr>
					<td>
						<a href="<?php echo esc_url( WPSTK_Admin::page_url( 'site-toolkit-' . $wpstk_module_id ) ); ?>">
...
<h2 class="wpstk-section-title"><?php echo esc_html__( 'All findings', 'site-toolkit' ); ?></h2>

<?php WPSTK_View::check_list( $wpstk_checks, true ); ?>

<h2 class="wpstk-section-title wpstk-no-print"><?php echo esc_html__( 'Scan history', 'site-toolkit' ); ?></h2>

<div class="wpstk-table-wrap wpstk-no-print">
	<table class="wp-list-table widefat striped">
		<thead>
			<tr>
				<th scope="col"><?php echo esc_html__( 'Finished', 'site-toolkit' ); ?></th>
			...
			<?php foreach ( $wpstk_history as $wpstk_row ) : ?>
				<tr<?php echo (int) $wpstk_row['id'] === (int) $wpstk_scan['id'] ? ' class="wpstk-row-current"' : ''; ?>>
					...
					<td>
						...
						?>
						"><?php echo esc_html__( 'CSV', 'site-toolkit' ); ?></a>

						<?php if ( WPSTK_Security::can( 'manage' ) ) : ?>
							<a class="button button-small wpstk-button-danger" data-wpstk-confirm="<?php echo esc_attr__( 'Delete this scan and its findings?', 'site-toolkit' ); ?>" href="
							<?php
							echo esc_url(
								wp_nonce_url(
									add_query_arg(
...
<p class="description wpstk-no-print">
	...
	printf(
		...
		esc_html__( 'History is kept for %1$s days, up to %2$s scans.', 'site-toolkit' ),
