...
class WPSTK_Redirects {
	...
	public static function create( $source, $target, $status_code = 301 ) {
		...
		if ( self::exists( $hash ) ) {
			return new WP_Error( 'wpstk_redirect_exists', __( 'A redirect for that address already exists.', 'site-toolkit' ) );
		}
		...
		if ( ! $inserted ) {
			return new WP_Error( 'wpstk_redirect_failed', __( 'The redirect could not be saved.', 'site-toolkit' ) );
		}
	...
	public static function update( $id, $source, $target, $status_code = 301, $enabled = true ) {
		...
		if ( $id <= 0 ) {
			return new WP_Error( 'wpstk_redirect_missing', __( 'That redirect could not be found.', 'site-toolkit' ) );
		}
	...
	private static function validate( $source, $target, $status_code ) {
		...
		if ( '' === $source || '' === $target ) {
			return new WP_Error( 'wpstk_redirect_invalid', __( 'A source path and a target address are both required.', 'site-toolkit' ) );
		}

		if ( self::resolve_target( $target ) === home_url( $source ) ) {
			return new WP_Error( 'wpstk_redirect_loop', __( 'The target address is the same as the source, which would create a redirect loop.', 'site-toolkit' ) );
		}
