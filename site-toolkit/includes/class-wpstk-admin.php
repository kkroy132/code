...
/**
 * Builds the Site Toolkit admin area.
 *
 * @since 1.0.0
 */
class WPSTK_Admin {
	...
	public function get_pages() {
		...
		$pages = array(
			array(
				'slug'  => self::MENU_SLUG,
				'title' => __( 'Dashboard', 'site-toolkit' ),
				'view'  => 'dashboard',
			),
			array(
				'slug'  => self::MENU_SLUG . '-audit',
				'title' => __( 'Full Audit', 'site-toolkit' ),
				'view'  => 'audit',
			),
		);
		...
		$pages[] = array(
			...
			'title' => __( 'Reports', 'site-toolkit' ),
			'view'  => 'reports',
		);

		$pages[] = array(
			'slug'  => self::MENU_SLUG . '-settings',
			'title' => __( 'Settings', 'site-toolkit' ),
			'view'  => 'settings',
		);
	...
	public function plugin_action_links( $links ) {
		...
		$dashboard = sprintf(
			...
			esc_html__( 'Dashboard', 'site-toolkit' )
		);

		$settings = sprintf(
			'<a href="%s">%s</a>',
			esc_url( admin_url( 'admin.php?page=' . self::MENU_SLUG . '-settings' ) ),
			esc_html__( 'Settings', 'site-toolkit' )
		);
	...
	public function enqueue_assets( $hook_suffix ) {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'    => array(
					'starting'    => __( 'Starting the audit…', 'site-toolkit' ),
					'running'     => __( 'Running', 'site-toolkit' ),
					'finished'    => __( 'Audit complete. Loading the report…', 'site-toolkit' ),
					'cancelled'   => __( 'The audit was cancelled.', 'site-toolkit' ),
					'failed'      => __( 'The audit stopped unexpectedly.', 'site-toolkit' ),
					'confirm'     => __( 'Cancel the running audit?', 'site-toolkit' ),
					'networkFail' => __( 'The connection to WordPress was lost. The audit can be resumed from this screen.', 'site-toolkit' ),
					/* translators: 1: current step number, 2: total number of steps. Filled in by JavaScript, not PHP. */
					'step'        => __( 'Step %1$d of %2$d', 'site-toolkit' ),
				),
			)
		);
	}
	...
	private function get_notice() {
		...
		if ( 'redirect-error' === $code ) {
			...
			return array(
				'type'    => 'error',
				'message' => '' !== $message ? $message : __( 'The redirect could not be saved.', 'site-toolkit' ),
			);
		}

		$messages = array(
			'scan-deleted'     => array( 'success', __( 'The scan was deleted.', 'site-toolkit' ) ),
			'entry-deleted'    => array( 'success', __( 'The entry was removed from the 404 log.', 'site-toolkit' ) ),
			'log-cleared'      => array( 'success', __( 'The 404 log was cleared.', 'site-toolkit' ) ),
			'data-cleared'     => array( 'success', __( 'All stored audit data was removed.', 'site-toolkit' ) ),
			'settings-reset'   => array( 'success', __( 'Settings were restored to their defaults.', 'site-toolkit' ) ),
			'redirect-created' => array( 'success', __( 'The redirect was created.', 'site-toolkit' ) ),
			'redirect-updated' => array( 'success', __( 'The redirect was updated.', 'site-toolkit' ) ),
			'redirect-deleted' => array( 'success', __( 'The redirect was deleted.', 'site-toolkit' ) ),
		);
