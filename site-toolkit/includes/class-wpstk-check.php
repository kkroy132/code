...
class WPSTK_Check {
	...
	public static function status_label( $status ) {
		switch ( $status ) {
			case 'critical':
				return __( 'Critical', 'site-toolkit' );
			case 'warning':
				return __( 'Warning', 'site-toolkit' );
			case 'recommendation':
				return __( 'Recommendation', 'site-toolkit' );
			case 'passed':
				return __( 'Passed', 'site-toolkit' );
			default:
	...
	public static function post_item( $post_id, $title, $detail = '' ) {
		$post_id = (int) $post_id;
		$title   = '' !== trim( (string) $title ) ? $title : __( '(no title)', 'site-toolkit' );
		...
		return self::item(
			array(
				...
				'link_label' => __( 'Edit', 'site-toolkit' ),
			)
		);
	}
	...
	public static function grade( $score ) {
		if ( null === $score ) {
			return array(
				...
				'description' => __( 'Run a full site audit to see where your site stands.', 'site-toolkit' ),
			);
		}
		...
		if ( $score >= 90 ) {
			return array(
				'key'         => 'excellent',
				'label'       => __( 'Excellent', 'site-toolkit' ),
				'description' => __( 'Everything the toolkit checks looks healthy.', 'site-toolkit' ),
			);
		}
		...
		if ( $score >= 75 ) {
			return array(
				'key'         => 'good',
				'label'       => __( 'Good', 'site-toolkit' ),
				'description' => __( 'Good — but some issues need attention.', 'site-toolkit' ),
			);
		}
		...
		if ( $score >= 50 ) {
			return array(
				'key'         => 'attention',
				'label'       => __( 'Needs attention', 'site-toolkit' ),
				'description' => __( 'Several checks did not pass. Start with the critical items.', 'site-toolkit' ),
			);
		}

		return array(
			'key'         => 'poor',
			'label'       => __( 'Needs work', 'site-toolkit' ),
			'description' => __( 'Many checks did not pass. The list below is ordered by severity.', 'site-toolkit' ),
		);
	}
}
