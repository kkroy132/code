...
class WPSTK_Security {
	...
	public static function capability( $context = 'view' ) {
		...
		/**
		 * Filters the capability required by Site Toolkit.
		 *
	...
	public static function require_cap( $context = 'view' ) {
		if ( ! self::can( $context ) ) {
			wp_die(
				esc_html__( 'You do not have permission to access Site Toolkit on this site.', 'site-toolkit' ),
				esc_html__( 'Permission denied', 'site-toolkit' ),
				array( 'response' => 403 )
			);
		}
	}
	...
	public static function verify_ajax( $context = 'run' ) {
		if ( ! check_ajax_referer( 'wpstk_ajax', 'nonce', false ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Your session has expired. Please reload the page and try again.', 'site-toolkit' ) ),
				403
			);
		...
		if ( ! self::can( $context ) ) {
			wp_send_json_error(
				array( 'message' => __( 'You do not have permission to perform this action.', 'site-toolkit' ) ),
				403
			);
		}
	}
