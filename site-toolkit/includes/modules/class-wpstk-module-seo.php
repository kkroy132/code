...
class WPSTK_Module_SEO extends WPSTK_Module {
	...
	public function get_description() {
		return __( 'Titles, meta descriptions, headings, canonical tags, social tags, indexability, sitemap and robots.txt.', 'site-toolkit' );
	}
	...
	public function get_tasks( $settings ) {
		return array(
			array(
				'id'    => 'content',
				'label' => __( 'Checking titles and meta descriptions', 'site-toolkit' ),
			),
			array(
				'id'    => 'pages',
				'label' => __( 'Inspecting rendered pages', 'site-toolkit' ),
			),
			array(
				'id'    => 'site',
				'label' => __( 'Checking sitemap, robots.txt and indexability', 'site-toolkit' ),
			),
		);
	}
				...
				if ( $title_length < self::TITLE_MIN ) {
					...
					$state['content']['title_short'] = $this->push_sample(
						...
						array(
							...
							sprintf( _n( '%d character', '%d characters', $title_length, 'site-toolkit' ), $title_length ),
						)
					);
			...
			if ( 'none' === $description['source'] || '' === $desc_value ) {
				...
				$state['content']['desc_short'] = $this->push_sample(
					...
					array(
						...
						$post['post_title'],
						/* translators: %d: number of characters. */
						sprintf( _n( '%d character', '%d characters', $desc_length, 'site-toolkit' ), $desc_length ),
					)
				);
			...
			if ( ! $response['success'] || $response['status'] >= 400 || '' === $response['body'] ) {
				...
				if ( '' === $state['pages']['first_error'] ) {
					$state['pages']['first_error'] = '' !== $response['message']
						...
						: sprintf( __( 'HTTP %d', 'site-toolkit' ), (int) $response['status'] );
				}
	...
	private function inspect_page( $entry, $html, &$state ) {
		...
		if ( empty( $canonicals ) ) {
			...
			$state['pages']['bad_canonical'] = $this->push_sample(
				...
				array( $post_id, $label, $url, __( 'More than one canonical tag', 'site-toolkit' ) )
			);
		} else {
			...
			if ( ! WPSTK_HTTP::is_requestable( $canonical ) ) {
				$state['pages']['bad_canonical'] = $this->push_sample(
					$state['pages']['bad_canonical'],
					array( $post_id, $label, $url, __( 'Canonical URL is not an absolute http(s) address', 'site-toolkit' ) )
				);
				...
				$state['pages']['bad_canonical'] = $this->push_sample(
					...
					array(
						...
						sprintf( __( 'Canonical points to another domain: %s', 'site-toolkit' ), WPSTK_Content::shorten( $canonical, 70 ) ),
					)
				);
			}
		}
		...
		if ( 0 === $h1_count ) {
			...
			$state['pages']['many_h1'] = $this->push_sample(
				...
				array(
					...
					sprintf( _n( '%d H1 heading', '%d H1 headings', $h1_count, 'site-toolkit' ), $h1_count ),
				)
			);
		}
		...
		if ( $jumped ) {
			$state['pages']['hierarchy'] = $this->push_sample(
				...
				array( $post_id, $label, $url, __( 'A heading level is skipped', 'site-toolkit' ) )
			);
		}
		...
		if ( ! empty( $missing_og ) ) {
			$state['pages']['no_og'] = $this->push_sample(
				...
				array(
					...
					sprintf( __( 'Missing: %s', 'site-toolkit' ), implode( ', ', $missing_og ) ),
				)
			);
		}
	...
	private function sample_urls( $limit ) {
		...
		$urls = array(
			array(
				'url'     => home_url( '/' ),
				'label'   => __( 'Home page', 'site-toolkit' ),
				'post_id' => 0,
			),
		);
		...
		foreach ( $posts as $post ) {
			...
			$urls[] = array(
				'url'     => $permalink,
				'label'   => '' !== $post->post_title ? $post->post_title : __( '(no title)', 'site-toolkit' ),
				'post_id' => (int) $post->ID,
			);
		}
	...
	private function finalize_content( $content, $settings ) {
		if ( empty( $content ) || empty( $content['total'] ) ) {
			return array(
				$this->skipped( 'meta_title', __( 'Meta titles', 'site-toolkit' ), __( 'No published content was found to analyse.', 'site-toolkit' ) ),
				$this->skipped( 'meta_description', __( 'Meta descriptions', 'site-toolkit' ), __( 'No published content was found to analyse.', 'site-toolkit' ) ),
			);
		}
		...
		if ( empty( $content['covered_all'] ) ) {
			$note = sprintf(
				...
				__( 'Analysed the first %1$d of %2$d published entries. Raise the "Maximum posts per scan" setting to cover more.', 'site-toolkit' ),
				$total,
		...
		foreach ( $content['title_missing'] as $row ) {
			$title_items[] = WPSTK_Check::post_item( $row[0], $row[1], __( 'No title', 'site-toolkit' ) );
		}
		...
		$checks[] = $this->check(
			array(
				'id'          => 'meta_title',
				'status'      => $status,
				'label'       => __( 'Meta titles', 'site-toolkit' ),
				'weight'      => 1.5,
				'summary'     => 'passed' === $status
					? sprintf(
						/* translators: %d: number of entries. */
						_n( '%d entry has a usable title.', 'All %d entries have a usable title.', $total, 'site-toolkit' ),
						$total
					)
					: sprintf(
						...
						__( '%1$d of %2$d entries have a title worth revisiting: %3$d empty, %4$d over 60 characters, %5$d under 30 characters.', 'site-toolkit' ),
						$title_problems,
						$total,
				...
				'why'         => __( 'The title is usually the first thing people read in search results and browser tabs. Empty titles give search engines nothing to work with, and very long ones are shortened before they are shown.', 'site-toolkit' ),
				'action'      => __( 'Give every entry a descriptive title of roughly 30 to 60 characters. If you use an SEO plugin, set the SEO title there; otherwise the post title is used.', 'site-toolkit' ),
				'note'        => $note,
				'items'       => $title_items,
		...
		if ( $counts['title_dupes'] > 0 ) {
			...
			$checks[] = $this->check(
				array(
					'id'          => 'duplicate_titles',
					'status'      => 'warning',
					'label'       => __( 'Duplicate titles', 'site-toolkit' ),
					'summary'     => sprintf(
						/* translators: %d: number of entries. */
						_n( '%d entry shares its title with another entry.', '%d entries share a title with another entry.', $counts['title_dupes'], 'site-toolkit' ),
						$counts['title_dupes']
					),
					'why'         => __( 'When several pages use the same title, search engines and visitors cannot tell them apart in a list of results.', 'site-toolkit' ),
					'action'      => __( 'Give each page a title that describes what makes it different.', 'site-toolkit' ),
					'note'        => $note,
					'items'       => $dupe_items,
			...
			$checks[] = $this->passed(
				'duplicate_titles',
				__( 'Duplicate titles', 'site-toolkit' ),
				__( 'Every title analysed is unique.', 'site-toolkit' ),
				__( 'Unique titles help search engines and visitors tell your pages apart.', 'site-toolkit' )
			);
		}
		...
		foreach ( $content['desc_missing'] as $row ) {
			$desc_items[] = WPSTK_Check::post_item( $row[0], $row[1], __( 'No description or excerpt', 'site-toolkit' ) );
		}
		...
		$checks[] = $this->check(
			array(
				'id'          => 'meta_description',
				'status'      => $status,
				'label'       => __( 'Meta descriptions', 'site-toolkit' ),
				'weight'      => 1.5,
				'summary'     => 'passed' === $status
					? __( 'Every entry analysed has a description of a reasonable length.', 'site-toolkit' )
					: sprintf(
						/* translators: 1: entries with description problems, 2: entries checked, 3: missing count, 4: too short count, 5: too long count. */
						__( '%1$d of %2$d entries could use a better description: %3$d with none, %4$d under 70 characters, %5$d over 160 characters.', 'site-toolkit' ),
						$desc_problems,
						$total,
						$counts['desc_missing'],
						$counts['desc_short'],
				...
				'why'         => __( 'Search engines often show the meta description underneath the title. Without one, they pick an arbitrary sentence from the page.', 'site-toolkit' ),
				'action'      => __( 'Write a description of roughly 70 to 160 characters for your most important pages. An excerpt is used when no SEO description is set.', 'site-toolkit' ),
				'note'        => $note,
				'items'       => $desc_items,
	...
	private function finalize_pages( $pages ) {
		$labels = array(
			'canonical' => __( 'Canonical URLs', 'site-toolkit' ),
			'h1'        => __( 'H1 headings', 'site-toolkit' ),
			'hierarchy' => __( 'Heading structure', 'site-toolkit' ),
			'og'        => __( 'Open Graph tags', 'site-toolkit' ),
			'noindex'   => __( 'Page level indexability', 'site-toolkit' ),
			'head'      => __( 'Rendered title and description tags', 'site-toolkit' ),
		);

		if ( empty( $pages ) || empty( $pages['fetched'] ) ) {
			$reason = __( 'The plugin could not load any pages from this site, so the rendered markup was not inspected.', 'site-toolkit' );

			if ( ! empty( $pages['first_error'] ) ) {
				$reason .= ' ' . sprintf(
					/* translators: %s: error message. */
					__( 'Last error: %s', 'site-toolkit' ),
					$pages['first_error']
				);
			}
		...
		$note    = sprintf(
			...
			_n( 'Based on %d page loaded from this site.', 'Based on %d pages loaded from this site.', $fetched, 'site-toolkit' ),
			$fetched
		);
		...
		$checks[] = $this->page_check(
			...
			__( 'A canonical URL tells search engines which address is the official one for a page.', 'site-toolkit' ),
			__( 'WordPress adds canonical tags automatically on most sites. If they are missing, check your theme header and any SEO plugin settings.', 'site-toolkit' ),
			__( 'Every sampled page has a single, valid canonical URL.', 'site-toolkit' ),
			$note,
			1.2
		);
		...
		$checks[] = $this->page_check(
			...
			$h1_items,
			$h1_status,
			__( 'One clear H1 describes what a page is about, for both readers and assistive technology.', 'site-toolkit' ),
			__( 'Make sure each page renders exactly one H1, usually the page title supplied by your theme.', 'site-toolkit' ),
			__( 'Every sampled page renders exactly one H1.', 'site-toolkit' ),
			$note,
			1.2
		);
		...
		$checks[] = $this->page_check(
			...
			__( 'Headings work best as an outline: H2 under H1, H3 under H2. Skipping a level makes the structure harder to follow with a screen reader.', 'site-toolkit' ),
			__( 'Reorder the headings on the listed pages so no level is skipped.', 'site-toolkit' ),
			__( 'No skipped heading levels were found on the sampled pages.', 'site-toolkit' ),
			$note
		);
		...
		$checks[] = $this->page_check(
			...
			__( 'Open Graph tags control the title, description and image used when a page is shared on social networks and chat apps.', 'site-toolkit' ),
			__( 'Add Open Graph tags through your theme or an SEO plugin. At minimum set og:title, og:description and og:image.', 'site-toolkit' ),
			__( 'The sampled pages all provide og:title, og:description and og:image.', 'site-toolkit' ),
			$note
		);
		...
		$checks[] = $this->page_check(
			...
			__( 'A noindex directive asks search engines to leave the page out of their results. That is sometimes intentional, but it is worth confirming.', 'site-toolkit' ),
			__( 'Open each listed page and remove the noindex setting if the page should be findable.', 'site-toolkit' ),
			__( 'None of the sampled pages asks search engines to skip it.', 'site-toolkit' ),
			$note,
			1.3
		);
		...
		$checks[] = $this->page_check(
			...
			$head_items,
			$head_status,
			__( 'This confirms what actually reaches the browser, which can differ from what is stored in the database.', 'site-toolkit' ),
			__( 'If a title tag is missing entirely, check that your theme calls wp_head() and that no plugin is removing it.', 'site-toolkit' ),
			__( 'Every sampled page outputs a title tag and a meta description.', 'site-toolkit' ),
			$note
		);
		...
		foreach ( (array) $rows as $row ) {
			...
			$items[] = WPSTK_Check::item(
				array(
					...
					'link_label' => $post_id > 0 ? __( 'Edit', 'site-toolkit' ) : '',
				)
			);
		}
			...
			array(
				...
				'summary'     => 'passed' === $status
					...
					: sprintf(
						...
						_n( '%d sampled page needs attention.', '%d sampled pages need attention.', $count, 'site-toolkit' ),
						$count
					),
				'why'         => $why,
				'action'      => $action,
		...
		if ( 0 === (int) $site['blog_public'] ) {
			$checks[] = $this->check(
				array(
					...
					'status'  => 'critical',
					'label'   => __( 'Search engine visibility', 'site-toolkit' ),
					'weight'  => 2.0,
					'summary' => __( 'This site asks search engines not to index it.', 'site-toolkit' ),
					'why'     => __( 'While this setting is on, WordPress asks search engines to stay away. That is right for a staging site and wrong for a live one.', 'site-toolkit' ),
					'action'  => __( 'If this site is live, turn off "Discourage search engines from indexing this site" in Settings → Reading.', 'site-toolkit' ),
					'items'   => array(
						WPSTK_Check::item(
							array(
								'label'      => __( 'Reading settings', 'site-toolkit' ),
								'detail'     => __( 'Discourage search engines is enabled', 'site-toolkit' ),
			...
			$checks[] = $this->passed(
				'search_visibility',
				__( 'Search engine visibility', 'site-toolkit' ),
				__( 'Search engines are allowed to index this site.', 'site-toolkit' ),
				__( 'A live site should let search engines in.', 'site-toolkit' )
			);
		}
		...
		if ( ! empty( $sitemap['found'] ) ) {
			$checks[] = $this->check(
				array(
					'id'      => 'sitemap',
					'status'  => 'passed',
					'label'   => __( 'XML sitemap', 'site-toolkit' ),
					'weight'  => 1.2,
					'summary' => __( 'A sitemap is reachable.', 'site-toolkit' ),
					'why'     => __( 'A sitemap gives search engines a complete list of the pages you want them to find.', 'site-toolkit' ),
					'items'   => array(
						WPSTK_Check::item(
							array(
								'label'      => $sitemap['url'],
								...
								'url'        => $sitemap['url'],
								'link'       => $sitemap['url'],
								'link_label' => __( 'View sitemap', 'site-toolkit' ),
							)
						),
					),
				)
			...
			$checks[] = $this->check(
				array(
					...
					'label'   => __( 'XML sitemap', 'site-toolkit' ),
					'weight'  => 1.2,
					'summary' => __( 'No sitemap was found at the usual addresses.', 'site-toolkit' ),
					'why'     => __( 'Without a sitemap, search engines have to discover your pages by following links, which is slower and less complete.', 'site-toolkit' ),
					'action'  => __( 'WordPress publishes /wp-sitemap.xml by default. If it is missing, check whether a plugin or theme disabled it, or enable the sitemap feature in your SEO plugin.', 'site-toolkit' ),
					'note'    => __( 'Addresses checked: /wp-sitemap.xml, /sitemap_index.xml, /sitemap.xml', 'site-toolkit' ),
				)
			);
		}
		...
		if ( empty( $robots['found'] ) ) {
			$checks[] = $this->check(
				array(
					'id'      => 'robots_txt',
					'status'  => 'warning',
					'label'   => __( 'robots.txt', 'site-toolkit' ),
					'summary' => sprintf(
						/* translators: %d: HTTP status code. */
						__( 'robots.txt could not be read (HTTP %d).', 'site-toolkit' ),
						(int) $robots['status']
					),
					'why'     => __( 'Crawlers request robots.txt before anything else. A server error there can slow down or block crawling.', 'site-toolkit' ),
					'action'  => __( 'Open /robots.txt in a browser. WordPress generates one automatically unless a real file or a redirect gets in the way.', 'site-toolkit' ),
				)
			);
			...
			$checks[] = $this->check(
				array(
					...
					'label'   => __( 'robots.txt', 'site-toolkit' ),
					'weight'  => 1.5,
					'summary' => __( 'robots.txt contains a rule that blocks the whole site.', 'site-toolkit' ),
					'why'     => __( 'A "Disallow: /" rule asks well behaved crawlers to skip every page on the site.', 'site-toolkit' ),
					'action'  => __( 'Remove the site-wide Disallow rule unless this site is meant to stay out of search results.', 'site-toolkit' ),
					'items'   => array(
						WPSTK_Check::item(
							array(
								'label'      => $robots['url'],
								'url'        => $robots['url'],
								'link'       => $robots['url'],
								'link_label' => __( 'View robots.txt', 'site-toolkit' ),
							)
						),
					),
				)
			...
			$checks[] = $this->check(
				array(
					...
					'summary' => empty( $robots['virtual'] )
						? __( 'A robots.txt file is served from the site root.', 'site-toolkit' )
						: __( 'WordPress is serving a generated robots.txt.', 'site-toolkit' ),
					'why'     => __( 'Crawlers read robots.txt before requesting anything else.', 'site-toolkit' ),
					'items'   => array(
						WPSTK_Check::item(
							array(
								'label'      => $robots['url'],
								'detail'     => ! empty( $robots['has_sitemap'] )
									? __( 'Includes a Sitemap directive', 'site-toolkit' )
									: __( 'No Sitemap directive', 'site-toolkit' ),
								'url'        => $robots['url'],
								'link'       => $robots['url'],
								'link_label' => __( 'View robots.txt', 'site-toolkit' ),
							)
