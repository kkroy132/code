...
class WPSTK_Export {
	...
	public static function export_csv() {
		...
		if ( ! $scan ) {
			wp_die(
				esc_html__( 'That scan could not be found.', 'site-toolkit' ),
				esc_html__( 'Export failed', 'site-toolkit' ),
				array( 'response' => 404 )
			);
		}

		$checks   = WPSTK_Scan_Store::sort_by_severity( WPSTK_Scan_Store::get_checks( $scan_id ) );
		$filename = sprintf( 'site-toolkit-scan-%d-%s.csv', $scan_id, gmdate( 'Ymd-His' ) );
		...
		if ( false === $output ) {
			wp_die( esc_html__( 'The export could not be created.', 'site-toolkit' ) );
		}

		self::write_row(
			$output,
			array(
				__( 'Section', 'site-toolkit' ),
				__( 'Check', 'site-toolkit' ),
				__( 'Severity', 'site-toolkit' ),
				__( 'Finding', 'site-toolkit' ),
				__( 'Why it matters', 'site-toolkit' ),
				__( 'Recommended action', 'site-toolkit' ),
				__( 'Affected items', 'site-toolkit' ),
				__( 'Examples', 'site-toolkit' ),
			)
		);
	...
	public static function delete_scan() {
		...
		self::redirect( 'site-toolkit-reports', 'scan-deleted' );
	}
	...
	public static function delete_not_found() {
		...
		self::redirect( 'site-toolkit-links', 'entry-deleted', array( 'tab' => '404' ) );
	}
	...
	public static function clear_not_found() {
		...
		self::redirect( 'site-toolkit-links', 'log-cleared', array( 'tab' => '404' ) );
	}
	...
	public static function clear_data() {
		...
		self::redirect( 'site-toolkit-settings', 'data-cleared' );
	}
	...
	public static function reset_settings() {
		...
		self::redirect( 'site-toolkit-settings', 'settings-reset' );
	}
	...
	public static function delete_redirect() {
		...
		self::redirect( 'site-toolkit-links', 'redirect-deleted', array( 'tab' => 'redirects' ) );
	}
	...
	public static function toggle_redirect() {
		...
		self::redirect( 'site-toolkit-links', 'redirect-updated', array( 'tab' => 'redirects' ) );
	}
